module.exports = {
  TEST_FISH: {
    999998: {
      _id: 999998,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 12,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 1,
      locations: [36, 35, 44],
      bestCatchPath: [2585],
      patch: 5.3,
    },
    999999: {
      _id: 999999,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: { 999998: 3 },
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 1,
      locations: [36, 35, 44],
      bestCatchPath: [2585],
      patch: 5.3,
    },
  },
  TEST_ITEMS: {
    999998: {
      _id: 999998,
      name_chs: '测试鱼',
      icon: 60034,
    },
    999999: {
      _id: 999999,
      name_chs: '测试鱼皇',
      icon: 60034,
    },
  },
  BAITS: [
    2585,
    2586,
    2587,
    2588,
    2589,
    2590,
    2591,
    2592,
    2593,
    2594,
    2595,
    2596,
    2597,
    2598,
    2599,
    2600,
    2601,
    2602,
    2603,
    2604,
    2605,
    2606,
    2607,
    2608,
    2609,
    2610,
    2611,
    2612,
    2613,
    2614,
    2615,
    2616,
    2617,
    2618,
    2619,
    2620,
    2621,
    2622,
    2623,
    2624,
    2625,
    2626,
    2627,
    2628,
    12704,
    12705,
    12706,
    12707,
    12708,
    12709,
    12710,
    12711,
    12712,
    20613,
    20614,
    20615,
    20616,
    20617,
    20618,
    20619,
    20675,
    20676,
    27582,
    27583,
    27584,
    27585,
    27586,
    27587,
    27588,
    27589,
    27590,
    27591,
    27592,
    28634,
    29714,
    29715,
    29716,
    29717,
    30136,
    30278,
    30279,
    30280,
    30281,
  ],
  FISH: {
    24995: {
      _id: 24995,
      previousWeatherSet: [7],
      weatherSet: [1],
      startHour: 5.5,
      endHour: 6.5,
      locations: [181],
      bestCatchPath: [20675],
      predators: {},
      patch: 4.5,
      folklore: 2505,
      collectable: false,
      fishEyes: false,
      snagging: null,
      hookset: 'Precision',
      tug: 'heavy',
      gig: null,
      aquarium: { water: 'freshwater', size: 1 },
      dataMissing: null,
    },
    28925: {
      _id: 28925,
      previousWeatherSet: [1, 2],
      weatherSet: [3, 4],
      startHour: 16,
      endHour: 24,
      locations: [206],
      bestCatchPath: [27589],
      predators: {},
      patch: 5.2,
      folklore: 2507,
      collectable: false,
      fishEyes: false,
      snagging: null,
      hookset: 'Powerful',
      tug: 'heavy',
      gig: null,
      aquarium: null,
      dataMissing: null,
    },
    27515: {
      _id: 27515,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      locations: [236],
      bestCatchPath: [27588],
      predators: {},
      patch: 5.0,
      folklore: null,
      collectable: true,
      fishEyes: false,
      snagging: null,
      hookset: 'Powerful',
      tug: 'medium',
      gig: null,
      aquarium: null,
      dataMissing: null,
    },
    // fix garland missing 4899 in 69
    4899: {
      _id: 4899,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 85,
      locations: [69, 74],
      bestCatchPath: [2619],
      patch: 2,
    },
    4911: {
      startHour: 17,
      endHour: 22,
    },
    4921: {
      weatherSet: [2, 1, 3, 4, 5],
    },
    4985: {
      startHour: 17,
      endHour: 10,
    },
    4999: {
      weatherSet: [1, 2, 3, 4],
    },
    7702: {
      weatherSet: [7, 8],
      startHour: 17,
      endHour: 7,
    },
    7902: {
      startHour: 17,
      endHour: 18.5,
      endHourText: '18:30',
    },
    7903: {
      weatherSet: [3, 4, 5],
      startHour: 3,
      endHour: 5,
    },
    7905: {
      weatherSet: [3, 4],
      startHour: 17,
      endHour: 19.5,
      endHourText: '19:30',
    },
    7906: {
      startHour: 17.5,
      endHour: 18,
      startHourText: '17:30',
    },
    7918: {
      weatherSet: [1],
      startHour: 17.5,
      endHour: 20,
      startHourText: '17:30',
    },
    7921: {
      weatherSet: [2, 1],
      startHour: 21.5,
      endHour: 3,
      startHourText: '21:30',
    },
    7923: {
      weatherSet: [4],
      startHour: 23,
      endHour: 4,
    },
    7927: {
      startHour: 16,
      endHour: 18.75,
      endHourText: '18:45',
    },
    7933: {
      startHour: 22,
      endHour: 4,
    },
    7937: {
      weatherSet: [7],
      startHour: 19.5,
      endHour: 2.75,
      startHourText: '19:30',
      endHourText: '2:45',
    },
    7947: {
      weatherSet: [4],
      startHour: 4.5,
      endHour: 10,
      startHourText: '4:30',
    },
    7948: {
      startHour: 23.5,
      endHour: 1.0833333333333333,
      startHourText: '23:30',
      endHourText: '1:05',
    },
    8756: {
      previousWeatherSet: [2, 1],
      weatherSet: [3, 4, 5],
      startHour: 20,
      endHour: 4,
    },
    8757: {
      previousWeatherSet: [],
      weatherSet: [2, 1],
      startHour: 19,
      endHour: 2,
    },
    8767: {
      previousWeatherSet: [2, 1],
      weatherSet: [4],
      startHour: 0,
      endHour: 3.3333333333333335,
      endHourText: '3:20',
    },
    8776: {
      previousWeatherSet: [2, 1],
      weatherSet: [4],
      startHour: 4.5,
      endHour: 11.5,
      startHourText: '4:30',
      endHourText: '11:30',
    },
    12786: {
      startHour: 18,
      endHour: 6,
    },
    12796: {
      startHour: 8,
      endHour: 18,
    },
    12803: {
      weatherSet: [3],
      startHour: 18,
      endHour: 3,
    },
    12828: {
      startHour: 22,
      endHour: 3.3333333333333335,
      endHourText: '3:20',
    },
    12832: {
      weatherSet: [2, 1],
      startHour: 9,
      endHour: 17,
    },
    13728: {
      weatherSet: [16],
      startHour: 0,
      endHour: 4,
      bestCatchPath: [2599, 109004937],
    },
    13731: {
      weatherSet: [2, 1],
      startHour: 15.5,
      endHour: 17,
      startHourText: '15:30',
    },
    13732: {
      weatherSet: [3, 4],
      startHour: 0,
      endHour: 6,
    },
    14218: {
      weatherSet: [9],
      startHour: 18,
      endHour: 24,
    },
    15638: {
      weatherSet: [3],
      startHour: 21,
      endHour: 2,
    },
    16746: {
      weatherSet: [16],
      startHour: 10,
      endHour: 18,
    },
    20034: {
      weatherSet: [1],
      startHour: 10,
      endHour: 18,
    },
    20085: {
      startHour: 16,
      endHour: 18.25,
      endHourText: '18:15',
    },
    20124: {
      startHour: 17,
      endHour: 10,
    },
    20524: {
      startHour: 8,
      endHour: 16,
    },
    // fix 黄昏珊瑚
    12795: {
      tug: 'light',
    },
    // fix 云水母
    5038: {
      tug: 'light',
    },
    8769: {
      bestCatchPath: [12711, 31004937],
    },
    7945: {
      weatherSet: [2, 1],
      startHour: 20,
      endHour: 4,
      bestCatchPath: [12704, 31004937],
    },
    7911: {
      bestCatchPath: [2594, 4948],
    },
    7946: {
      bestCatchPath: [12711, 31004937],
    },
    16745: {
      bestCatchPath: [12712, 12716, 12724],
    },
    4893: {
      tug: 'medium',
    },
    30492: {
      tug: 'light',
    },
    24888: {
      previousWeatherSet: [],
    },
    12750: {
      tug: 'medium',
    },
    12808: {
      hookset: 'Precision',
      tug: 'light',
    },
    12772: {
      snagging: true,
    },
    20073: {
      weatherSet: [3, 4, 5, 11],
    },
    20021: {
      bestCatchPath: [20675],
    },
    20060: {
      bestCatchPath: [20614],
    },
    27412: {
      snagging: false,
    },
    12745: {
      tug: 'medium',
    },
    4912: {
      tug: 'medium',
    },
    12734: {
      tug: 'light',
    },
    20048: {
      tug: 'medium',
    },
    4977: {
      bestCatchPath: [2600],
    },
    5043: {
      bestCatchPath: [2600, 4977],
    },
    23073: {
      _id: 23073,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 776,
      locations: [116],
      bestCatchPath: [28634],
      patch: 4.3,
    },
    23075: {
      _id: 23075,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 778,
      locations: [131],
      bestCatchPath: [28634],
      patch: 4.3,
    },
    17565: {
      patch: 3.5,
      hookset: 'Precision',
      bestCatchPath: [28634],
    },
    12817: {
      hookset: 'Powerful',
      tug: 'heavy',
    },
    4997: {
      _id: 4997,
      hookset: 'Powerful',
    },
    12771: {
      _id: 12771,
      hookset: 'Precision',
      tug: 'light',
    },
    17564: {
      _id: 17564,
      patch: 3.5,
      hookset: 'Precision',
      tug: 'light',
    },
    // '12776': {
    //   bestCatchPathGroup: {
    //     '140': {
    //       bestCatchPath: [12710],
    //     },
    //     '141': {
    //       bestCatchPath: [12710],
    //     },
    //     '142': {
    //       bestCatchPath: [30136],
    //     },
    //     '146': {
    //       bestCatchPath: [30136],
    //     },
    //   },
    // },
    12815: {
      bestCatchPath: [30136, 12776],
    },
    12816: {
      bestCatchPath: [30136, 12776],
    },
    12827: {
      bestCatchPath: [30136, 12776],
    },
    16744: {
      bestCatchPath: [30136, 12776],
    },
    17589: {
      bestCatchPath: [30136, 12776],
    },
    17562: {
      patch: 3.5,
    },
    17563: {
      patch: 3.5,
    },
    17566: {
      patch: 3.5,
    },
    22393: {
      bestCatchPath: [20617],
      bestCatchPathExtra: [20617, 20112],
    },
    12764: {
      hookset: 'Precision',
      tug: 'light',
    },
    29673: {
      patch: 5.25,
    },
    29678: {
      patch: 5.25,
    },
    12719: {
      _id: 12719,
      hookset: 'Powerful',
      tug: 'medium',
    },
    16747: {
      _id: 16747,
      previousWeatherSet: [3, 4, 11],
      weatherSet: [1],
    },
    12738: {
      _id: 12738,
      hookset: 'Powerful',
      tug: 'medium',
    },
    56004991: {
      _id: 56004991,
      previousWeatherSet: [],
      weatherSet: [7, 8],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 113,
      locations: [56],
      bestCatchPath: [2620],
      patch: 2,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    99004991: {
      _id: 99004991,
      previousWeatherSet: [],
      weatherSet: [7, 8],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 113,
      locations: [99],
      bestCatchPath: [2620],
      patch: 2,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    103005005: {
      _id: 103005005,
      previousWeatherSet: [],
      weatherSet: [4],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 136,
      locations: [103],
      bestCatchPath: [2599, 4978],
      patch: 2,
    },
    89005005: {
      _id: 89005005,
      previousWeatherSet: [],
      weatherSet: [4],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 136,
      locations: [89],
      bestCatchPath: [2599, 4978],
      patch: 2,
    },
    93005005: {
      _id: 93005005,
      previousWeatherSet: [],
      weatherSet: [4],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 136,
      locations: [93],
      bestCatchPath: [2599, 4978],
      patch: 2,
    },
    175020036: {
      _id: 175020036,
      previousWeatherSet: [],
      weatherSet: [1],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 574,
      locations: [175],
      bestCatchPath: [20615],
      patch: 4,
      folklore: null,
      collectable: true,
      gig: null,
      aquarium: null,
    },
    198020036: {
      _id: 198020036,
      previousWeatherSet: [],
      weatherSet: [1],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 574,
      locations: [198],
      bestCatchPath: [20615],
      patch: 4,
      folklore: null,
      collectable: true,
      gig: null,
      aquarium: null,
    },
    182020058: {
      _id: 182020058,
      previousWeatherSet: [],
      weatherSet: [3, 4],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 518,
      locations: [182, 186],
      bestCatchPath: [20613],
      patch: 4,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    185020058: {
      _id: 185020058,
      previousWeatherSet: [],
      weatherSet: [3, 4],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 518,
      locations: [185],
      bestCatchPath: [20614, 20064],
      patch: 4,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    36004885: {
      _id: 36004885,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 45,
      locations: [36, 35, 37, 38, 44, 45, 51, 60],
      bestCatchPath: [2585, 4869],
      patch: 2,
    },
    46004885: {
      _id: 46004885,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 45,
      locations: [46, 59, 104, 70],
      bestCatchPath: [2587, 4874],
      patch: 2,
    },
    38004903: {
      _id: 38004903,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 111,
      locations: [38, 51, 62, 74],
      bestCatchPath: [2585, 4869],
      patch: 2,
    },
    52004903: {
      _id: 52004903,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'medium',
      dataMissing: false,
      anglerFishId: 111,
      locations: [52],
      bestCatchPath: [2596, 4898],
      patch: 2,
    },
    42004952: {
      _id: 42004952,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 47,
      locations: [42, 50, 14, 72, 73, 106],
      bestCatchPath: [2594, 4948],
      patch: 2,
    },
    2004952: {
      _id: 2004952,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 47,
      locations: [2, 6],
      bestCatchPath: [2586, 4927],
      patch: 2,
    },
    8004952: {
      _id: 8004952,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 47,
      locations: [8, 11, 13],
      bestCatchPath: [2592, 4942],
      patch: 2,
    },
    19004963: {
      _id: 19004963,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 64,
      locations: [19],
      bestCatchPath: [2586, 4927],
      patch: 2,
    },
    105004963: {
      _id: 105004963,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 64,
      locations: [105],
      bestCatchPath: [2592, 4942],
      patch: 2,
    },
    55004975: {
      _id: 55004975,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 86,
      locations: [55],
      bestCatchPath: [2594, 4948],
      patch: 2,
    },
    4004975: {
      _id: 4004975,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 86,
      locations: [4, 92, 17, 105, 25],
      bestCatchPath: [2592, 4942],
      patch: 2,
    },
    26005009: {
      _id: 26005009,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 144,
      locations: [26, 28, 30],
      bestCatchPath: [2586, 4927],
      patch: 2,
    },
    32005009: {
      _id: 32005009,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 144,
      locations: [32],
      bestCatchPath: [12711, 31004937],
      patch: 2,
    },
    31004937: {
      _id: 31004937,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 24,
      locations: [31, 28, 30, 32, 29],
      bestCatchPath: [12711],
      patch: 2,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    109004937: {
      _id: 109004937,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 24,
      locations: [109, 111, 112, 114, 115],
      bestCatchPath: [2599],
      patch: 2,
      folklore: null,
      collectable: false,
      gig: null,
      aquarium: null,
    },
    5010: {
      // locations: [29],
      bestCatchPath: [12711, 31004937],
    },
    5025: {
      bestCatchPath: [12704, 31004937],
    },
    12726: {
      // locations: [111],
      bestCatchPath: [2599, 109004937],
    },
    12748: {
      // locations: [115],
      bestCatchPath: [2599, 109004937],
    },
    120012802: {
      _id: 120012802,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 18,
      endHour: 20.5,
      endHourText: '20:30',
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'heavy',
      dataMissing: true,
      anglerFishId: 402,
      locations: [120],
      bestCatchPath: [12704, 12722],
      patch: 3,
      folklore: null,
      collectable: true,
      gig: null,
      aquarium: null,
    },
    135012802: {
      _id: 135012802,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 18,
      endHour: 20.5,
      endHourText: '20:30',
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Powerful',
      tug: 'heavy',
      dataMissing: true,
      anglerFishId: 402,
      locations: [135],
      bestCatchPath: [12707, 12730],
      patch: 3,
      folklore: null,
      collectable: true,
      gig: null,
      aquarium: null,
    },
    182020057: {
      _id: 182020057,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 532,
      locations: [182],
      bestCatchPath: [20615, 20056],
      patch: 4,
    },
    184020057: {
      _id: 184020057,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'light',
      dataMissing: true,
      anglerFishId: 532,
      locations: [184, 188],
      bestCatchPath: [20613],
      patch: 4,
    },
    175020126: {
      _id: 175020126,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 576,
      bait: 20127,
      locations: [175, 178],
      bestCatchPath: [20614, 20127],
      patch: 4,
    },
    180020126: {
      _id: 180020126,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: null,
      tug: 'medium',
      dataMissing: true,
      anglerFishId: 576,
      locations: [180],
      bestCatchPath: [20615],
      patch: 4,
    },
    175020132: {
      _id: 175020132,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 580,
      locations: [175],
      bestCatchPath: [20614, 20127],
      patch: 4,
    },
    180020132: {
      _id: 180020132,
      previousWeatherSet: [],
      weatherSet: [],
      startHour: 0,
      endHour: 24,
      predators: {},
      fishEyes: false,
      snagging: false,
      hookset: 'Precision',
      tug: 'light',
      dataMissing: true,
      anglerFishId: 580,
      locations: [180],
      bestCatchPath: [20615],
      patch: 4,
    },
    27494: {
      fishEyes: 240,
    },
    12784: {
      bestCatchPath: [12708, 12753],
      bestCatchPathExtra: [12708, 12805],
    },
    28065: {
      startHour: 18,
      endHour: 22,
    },
    12753: {
      bestCatchPath: [28634],
    },
    12810: {
      bestCatchPath: [28634],
    },
    12798: {
      tug: 'medium',
    },
    20108: {
      snagging: false,
    },
    4994: {
      bestCatchPath: [2601],
    },
    12811: {
      tug: 'light',
      bestCatchPath: [12706],
    },
    15632: {
      bestCatchPath: [12707, 12780],
    },
    4930: {
      tug: 'light',
    },
    12760: {
      tug: 'medium',
    },
    4887: {
      tug: 'medium'
    },
    4935: {
      tug: 'light'
    },
    32050: {
      previousWeatherSet: [3, 4],
      weatherSet: [4],
    },
    32054: {
      startHour: 8,
      endHour: 10,
    }
  },
  COLLECTABLE_FISH_ITEM_ID: [
    12713,
    12721,
    12724,
    12726,
    12739,
    12742,
    12761,
    12767,
    12768,
    12774,
    12784,
    12792,
    12802,
    12804,
    12814,
    12821,
    12825,
    12827,
    12828,
    12830,
    12831,
    12833,
    12834,
    12837,
    13729,
    14211,
    17562,
    17563,
    17564,
    17565,
    17566,
    20087,
    20098,
    20118,
    20036,
    20028,
    20044,
    20104,
    20019,
    20021,
    20024,
    20030,
    20040,
    20051,
    20074,
    20100,
    20142,
    20785,
    20786,
    20787,
    20788,
    20789,
    23071,
    23072,
    23073,
    23074,
    23075,
    24557,
    24558,
    24559,
    24560,
    24561,
    27431,
    27455,
    27480,
    27491,
    27492,
    27442,
    27432,
    27456,
    27467,
    27481,
    27494,
    27510,
    27515,
    27502,
    27508,
    28189,
    28190,
    28191,
    28192,
    28193,
    30593,
    30488,
    30489,
    30490,
    30491,
    30492,
  ],
  OCEAN_FISHING_FISH: {
    28937: {
      _id: 28937,
      name_chs: '加拉迪翁虾虎',
      icon: 29003,
      tug: 'light',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 7,
      points: 10,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    28938: {
      _id: 28938,
      name_chs: '加拉迪翁鳀鱼',
      icon: 29034,
      tug: 'light',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 9,
      points: 11,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    28939: {
      _id: 28939,
      name_chs: '玫瑰鲷鱼',
      icon: 29009,
      tug: 'light',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 14,
      points: 34,
      doubleHook: [3, 4],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    28940: {
      _id: 28940,
      name_chs: '三角刺鱼',
      icon: 29152,
      tug: 'light',
      hookset: null,
      biteTimeMin: 10,
      biteTimeMax: 23,
      points: 43,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [3, 4],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    28941: {
      _id: 28941,
      name_chs: '阳光蝴蝶鱼',
      icon: 29016,
      tug: 'light',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 8,
      points: 10,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    28942: {
      _id: 28942,
      name_chs: '暗淡鲨',
      icon: 29011,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 16,
      biteTimeMax: 30,
      points: 34,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [8],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29718: {
      _id: 29718,
      name_chs: '海投刀',
      icon: 29042,
      tug: 'light',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 13,
      points: 27,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [16],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29719: {
      _id: 29719,
      name_chs: '碧玉头',
      icon: 29043,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 10,
      biteTimeMax: 23,
      points: 40,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [3, 4],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29720: {
      _id: 29720,
      name_chs: '梅尔托尔龙虾',
      icon: 29013,
      tug: 'light',
      hookset: null,
      biteTimeMin: 17,
      biteTimeMax: 27,
      points: 45,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29721: {
      _id: 29721,
      name_chs: '天堂鱼',
      icon: 29033,
      tug: 'light',
      hookset: null,
      biteTimeMin: 16,
      biteTimeMax: 20,
      points: 50,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29722: {
      _id: 29722,
      name_chs: '尸鬼梭鱼',
      icon: 29035,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 7,
      points: 10,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [5, 6],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29723: {
      _id: 29723,
      name_chs: '猎豹鳗',
      icon: 29126,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 15,
      points: 14,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [7, 8],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29724: {
      _id: 29724,
      name_chs: '海中爆弹',
      icon: 29046,
      tug: 'light',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 8,
      points: 27,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [6],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29725: {
      _id: 29725,
      name_chs: '莫莫拉·莫拉',
      icon: 29021,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 14,
      biteTimeMax: 20,
      points: 22,
      doubleHook: [2],
      bait: 29716,
      baitExtra: 29722,
      bestCatchPath: [29715, 29722],
      bestCatchPathExtra: [29716],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29726: {
      _id: 29726,
      name_chs: '梅尔托尔蝴蝶鱼',
      icon: 29167,
      tug: 'light',
      hookset: null,
      biteTimeMin: 17,
      biteTimeMax: 27,
      points: 51,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29727: {
      _id: 29727,
      name_chs: '步兵剑',
      icon: 29022,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 13,
      points: 49,
      doubleHook: [2],
      bait: 29722,
      baitExtra: null,
      bestCatchPath: [29715, 29722],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29728: {
      _id: 29728,
      name_chs: '罗塔诺梭子鰆',
      icon: 29025,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 10,
      points: 13,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [14],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29729: {
      _id: 29729,
      name_chs: '罗塔诺沙丁鱼',
      icon: 29004,
      tug: 'light',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 10,
      points: 10,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29730: {
      _id: 29730,
      name_chs: '深海平鱼',
      icon: 29048,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 12,
      points: 15,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [11, 14],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29731: {
      _id: 29731,
      name_chs: '深红钓鮟鱇',
      icon: 29049,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 14,
      points: 32,
      doubleHook: [4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29732: {
      _id: 29732,
      name_chs: '灯鱼',
      icon: 29516,
      tug: 'light',
      hookset: null,
      biteTimeMin: 11,
      biteTimeMax: 16,
      points: 47,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29733: {
      _id: 29733,
      name_chs: '魔鳗',
      icon: 29045,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 8,
      biteTimeMax: 15,
      points: 38,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [3, 4],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29734: {
      _id: 29734,
      name_chs: '青色章鱼',
      icon: 29051,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 17,
      biteTimeMax: 27,
      points: 59,
      doubleHook: [4],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 13,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29735: {
      _id: 29735,
      name_chs: '铬铁锤头鲨',
      icon: 29026,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 15,
      biteTimeMax: 28,
      points: 32,
      doubleHook: [4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29736: {
      _id: 29736,
      name_chs: '浮冰鱼',
      icon: 29310,
      tug: 'light',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 7,
      points: 13,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [15, 16],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29737: {
      _id: 29737,
      name_chs: '巨乌贼',
      icon: 29027,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 10,
      points: 11,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29738: {
      _id: 29738,
      name_chs: '放浪神之石',
      icon: 29100,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 12,
      biteTimeMax: 24,
      points: 11,
      doubleHook: [2],
      bait: 29716,
      baitExtra: 29718,
      bestCatchPath: [29714, 29718],
      bestCatchPathExtra: [29716],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29739: {
      _id: 29739,
      name_chs: '拉诺西亚水母',
      icon: 29755,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 10,
      doubleHook: [3, 4],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 15,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29740: {
      _id: 29740,
      name_chs: '蓬松海龙',
      icon: 29053,
      tug: 'light',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 20,
      points: 35,
      doubleHook: [4],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 16,
      fishTipType: null,
      notAvailableWeatherSet: [3, 4],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29741: {
      _id: 29741,
      name_chs: '蜘蛛蟹',
      icon: 29111,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 21,
      points: 36,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29742: {
      _id: 29742,
      name_chs: '暗鹦鹉螺',
      icon: 29811,
      tug: 'light',
      hookset: null,
      biteTimeMin: 17,
      biteTimeMax: 24,
      points: 46,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29743: {
      _id: 29743,
      name_chs: '古老恐鱼',
      icon: 29019,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 16,
      points: 52,
      doubleHook: [2],
      bait: 29718,
      baitExtra: null,
      bestCatchPath: [29714, 29718],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29744: {
      _id: 29744,
      name_chs: '醉鱼',
      icon: 29373,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 16,
      biteTimeMax: 27,
      points: 253,
      doubleHook: [1],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: {
        28938: 3,
      },
      bonus: 0,
      fishTipType: 2,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29745: {
      _id: 29745,
      name_chs: '小利维亚桑',
      icon: 29147,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 16,
      biteTimeMax: 27,
      points: 204,
      doubleHook: [1],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: {
        29727: 1,
      },
      bonus: 0,
      fishTipType: 2,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29746: {
      _id: 29746,
      name_chs: '海铠靴',
      icon: 29077,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 11,
      biteTimeMax: 18,
      points: 204,
      doubleHook: [1],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: {
        29731: 2,
      },
      bonus: 0,
      fishTipType: 2,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29747: {
      _id: 29747,
      name_chs: '海流星',
      icon: 29157,
      tug: 'heavy',
      hookset: 'Precision',
      biteTimeMin: 16,
      biteTimeMax: 27,
      points: 226,
      doubleHook: [1],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: {
        29743: 1,
      },
      bonus: 0,
      fishTipType: 2,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29748: {
      _id: 29748,
      name_chs: '锤爪龙虾',
      icon: 29082,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 69,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [244],
      patch: 5.2,
    },
    29749: {
      _id: 29749,
      name_chs: '天堂之钥',
      icon: 29069,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 67,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [238],
      patch: 5.2,
    },
    29750: {
      _id: 29750,
      name_chs: '幽灵鲨',
      icon: 29065,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 4,
      points: 78,
      doubleHook: [4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [238],
      patch: 5.2,
    },
    29751: {
      _id: 29751,
      name_chs: '流银刃',
      icon: 29759,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 6,
      points: 213,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [238],
      patch: 5.2,
    },
    29752: {
      _id: 29752,
      name_chs: '海神印',
      icon: 29073,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 4,
      points: 71,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [238],
      patch: 5.2,
    },
    29753: {
      _id: 29753,
      name_chs: '钉牡蛎',
      icon: 29067,
      tug: 'light',
      hookset: null,
      biteTimeMin: 8,
      biteTimeMax: 9,
      points: 222,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [238],
      patch: 5.2,
    },
    29754: {
      _id: 29754,
      name_chs: '巨食鮟鱇',
      icon: 29070,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 7,
      points: 78,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [238],
      patch: 5.2,
    },
    29755: {
      _id: 29755,
      name_chs: '秘银君',
      icon: 29078,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 5,
      points: 196,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [240],
      patch: 5.2,
    },
    29756: {
      _id: 29756,
      name_chs: '灵敏舞者',
      icon: 28001,
      tug: 'light',
      hookset: null,
      biteTimeMin: 11,
      biteTimeMax: 15,
      points: 444,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [238],
      patch: 5.2,
    },
    29757: {
      _id: 29757,
      name_chs: '海荨麻',
      icon: 29434,
      tug: 'light',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 8,
      points: 156,
      doubleHook: [4],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 15,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [240],
      patch: 5.2,
    },
    29758: {
      _id: 29758,
      name_chs: '巨大枪鱼',
      icon: 29095,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: null,
      points: 127,
      doubleHook: [2],
      bait: 29761,
      baitExtra: null,
      bestCatchPath: [29716, 29761],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [240],
      patch: 5.2,
    },
    29759: {
      _id: 29759,
      name_chs: '海难船帆',
      icon: 29084,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 59,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [240],
      patch: 5.2,
    },
    29760: {
      _id: 29760,
      name_chs: '太阳神面纱',
      icon: 29161,
      tug: 'light',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 6,
      points: 69,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [240],
      patch: 5.2,
    },
    29761: {
      _id: 29761,
      name_chs: '高级以太药虱',
      icon: 29089,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 4,
      points: 65,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [240],
      patch: 5.2,
    },
    29762: {
      _id: 29762,
      name_chs: '浮游碟鱼',
      icon: 29661,
      tug: 'light',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 10,
      points: 222,
      doubleHook: [4],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 15,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [242],
      patch: 5.2,
    },
    29763: {
      _id: 29763,
      name_chs: '以太海龙',
      icon: 29074,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 245,
      doubleHook: [2],
      bait: 29761,
      baitExtra: null,
      bestCatchPath: [29716, 29761],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 16,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [240],
      patch: 5.2,
    },
    29764: {
      _id: 29764,
      name_chs: '珊瑚海龙',
      icon: 29155,
      tug: 'light',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 10,
      points: 189,
      doubleHook: [4],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 16,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [244],
      patch: 5.2,
    },
    29765: {
      _id: 29765,
      name_chs: '无赖龙',
      icon: 28002,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 4,
      points: 345,
      doubleHook: [2],
      bait: 29761,
      baitExtra: null,
      bestCatchPath: [29716, 29761],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [240],
      patch: 5.2,
    },
    29766: {
      _id: 29766,
      name_chs: '人鱼发',
      icon: 29072,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 94,
      doubleHook: [4],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 13,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [238],
      patch: 5.2,
    },
    29767: {
      _id: 29767,
      name_chs: '清道夫',
      icon: 29085,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 6,
      points: 216,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [242],
      patch: 5.2,
    },
    29768: {
      _id: 29768,
      name_chs: '沉寂者',
      icon: 29156,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 6,
      points: 89,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [242],
      patch: 5.2,
    },
    29769: {
      _id: 29769,
      name_chs: '深海鳗',
      icon: 29088,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 8,
      points: 81,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [242],
      patch: 5.2,
    },
    29770: {
      _id: 29770,
      name_chs: '处刑者',
      icon: 29092,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 8,
      points: 216,
      doubleHook: [4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [242],
      patch: 5.2,
    },
    29771: {
      _id: 29771,
      name_chs: '狂野海胆',
      icon: 29813,
      tug: 'light',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 6,
      points: 79,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [244],
      patch: 5.2,
    },
    29772: {
      _id: 29772,
      name_chs: '尖吻鲈',
      icon: 29546,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 6,
      points: 95,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [242],
      patch: 5.2,
    },
    29773: {
      _id: 29773,
      name_chs: '幻纱披风',
      icon: 29374,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 5,
      biteTimeMax: 7,
      points: 198,
      doubleHook: [4],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 13,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [244],
      patch: 5.2,
    },
    29774: {
      _id: 29774,
      name_chs: '蜗牛鱼',
      icon: 28003,
      tug: 'light',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 5,
      points: 246,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [242],
      patch: 5.2,
    },
    29775: {
      _id: 29775,
      name_chs: '阿龙纳斯',
      icon: 29816,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 95,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [242],
      patch: 5.2,
    },
    29776: {
      _id: 29776,
      name_chs: '尾骨鱼',
      icon: 29093,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 79,
      doubleHook: [2],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [244],
      patch: 5.2,
    },
    29777: {
      _id: 29777,
      name_chs: '剪碎手巴塞洛缪',
      icon: 29075,
      tug: 'light',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 5,
      points: 221,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [244],
      patch: 5.2,
    },
    29778: {
      _id: 29778,
      name_chs: '巡游者',
      icon: 29066,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 5,
      points: 79,
      doubleHook: [2],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [244],
      patch: 5.2,
    },
    29779: {
      _id: 29779,
      name_chs: '寄生幸存者',
      icon: 29086,
      tug: 'light',
      hookset: null,
      biteTimeMin: 2,
      biteTimeMax: 4,
      points: 69,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [240],
      patch: 5.2,
    },
    29780: {
      _id: 29780,
      name_chs: '浪子鱼',
      icon: 29331,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 3,
      biteTimeMax: 8,
      points: 95,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [242],
      patch: 5.2,
    },
    29781: {
      _id: 29781,
      name_chs: '守领鳍龙',
      icon: 29162,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 4,
      biteTimeMax: 7,
      points: 79,
      doubleHook: [3, 4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 0,
      locations: [244],
      patch: 5.2,
    },
    29782: {
      _id: 29782,
      name_chs: '漏斗鲨',
      icon: 29731,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 6,
      biteTimeMax: 10,
      points: 213,
      doubleHook: [4],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 14,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [238],
      patch: 5.2,
    },
    29783: {
      _id: 29783,
      name_chs: '坠天鱼',
      icon: 28004,
      tug: 'medium',
      hookset: null,
      biteTimeMin: 7,
      biteTimeMax: 8,
      points: 374,
      doubleHook: [2],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: null,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [244],
      patch: 5.2,
    },
    29784: {
      _id: 29784,
      name_chs: '幻光巨齿鲨',
      icon: 28005,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 20,
      biteTimeMax: 35,
      points: 100,
      doubleHook: [1],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: 1,
      notAvailableWeatherSet: [1],
      time: 0,
      locations: [237],
      patch: 5.2,
    },
    29785: {
      _id: 29785,
      name_chs: '幻光盘丽鱼',
      icon: 28006,
      tug: 'heavy',
      hookset: 'Precision',
      biteTimeMin: 20,
      biteTimeMax: 35,
      points: 100,
      doubleHook: [1],
      bait: 29715,
      baitExtra: null,
      bestCatchPath: [29715],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: 1,
      notAvailableWeatherSet: [1],
      time: 0,
      locations: [239],
      patch: 5.2,
    },
    29786: {
      _id: 29786,
      name_chs: '幻光海马',
      icon: 28007,
      tug: 'heavy',
      hookset: 'Precision',
      biteTimeMin: 20,
      biteTimeMax: 35,
      points: 100,
      doubleHook: [1],
      bait: 29714,
      baitExtra: null,
      bestCatchPath: [29714],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: 1,
      notAvailableWeatherSet: [1],
      time: 0,
      locations: [243],
      patch: 5.2,
    },
    29787: {
      _id: 29787,
      name_chs: '幻光鲈',
      icon: 28008,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 21,
      biteTimeMax: 35,
      points: 100,
      doubleHook: [1],
      bait: 29716,
      baitExtra: null,
      bestCatchPath: [29716],
      bestCatchPathExtra: [],
      predators: null,
      bonus: 0,
      fishTipType: 1,
      notAvailableWeatherSet: [1],
      time: 0,
      locations: [241],
      patch: 5.2,
    },
    29788: {
      _id: 29788,
      name_chs: '索蒂斯',
      icon: 28009,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 11,
      points: 500,
      doubleHook: [1],
      bait: 2603,
      baitExtra: null,
      bestCatchPath: [2603],
      bestCatchPathExtra: [],
      predators: {
        29749: 2,
        29752: 1,
      },
      bonus: 0,
      fishTipType: 3,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [238],
      patch: 5.2,
    },
    29789: {
      _id: 29789,
      name_chs: '珊瑚蝠鲼',
      icon: 28010,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 9,
      biteTimeMax: 10,
      points: 500,
      doubleHook: [1],
      bait: 2613,
      baitExtra: null,
      bestCatchPath: [2613],
      bestCatchPathExtra: [],
      predators: {
        29758: 2,
      },
      bonus: 0,
      fishTipType: 3,
      notAvailableWeatherSet: [],
      time: 3,
      locations: [240],
      patch: 5.2,
    },
    29790: {
      _id: 29790,
      name_chs: '石骨鱼',
      icon: 28011,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 10,
      biteTimeMax: 12,
      points: 500,
      doubleHook: [1],
      bait: 2591,
      baitExtra: null,
      bestCatchPath: [2591],
      bestCatchPathExtra: [],
      predators: {
        29768: 1,
        29769: 1,
      },
      bonus: 0,
      fishTipType: 3,
      notAvailableWeatherSet: [],
      time: 2,
      locations: [242],
      patch: 5.2,
    },
    29791: {
      _id: 29791,
      name_chs: '依拉丝莫龙',
      icon: 28012,
      tug: 'heavy',
      hookset: null,
      biteTimeMin: 10,
      biteTimeMax: 11,
      points: 500,
      doubleHook: [1],
      bait: 2619,
      baitExtra: null,
      bestCatchPath: [2619],
      bestCatchPathExtra: [],
      predators: {
        29781: 3,
      },
      bonus: 0,
      fishTipType: 3,
      notAvailableWeatherSet: [],
      time: 1,
      locations: [244],
      patch: 5.2,
    },
  },
  OCEAN_FISHING_BONUS: {
    0: {
      _id: 0,
      objective: null,
      requirement: null,
      icon: 0,
      order: 0,
    },
    1: {
      _id: 1,
      objective: '千鱼祝福',
      requirement: '达成条件：获得的渔分超过2500。',
      icon: 65901,
      order: 1,
    },
    2: {
      _id: 2,
      objective: '万鱼贺喜',
      requirement: '达成条件：获得的渔分超过5000。',
      icon: 65902,
      order: 2,
    },
    3: {
      _id: 3,
      objective: '珍鱼',
      requirement: '达成条件：钓到一条★★★★鱼。',
      icon: 65903,
      order: 3,
    },
    4: {
      _id: 4,
      objective: '传说鱼',
      requirement: '达成条件：钓到一条★★★★★鱼。',
      icon: 65904,
      order: 4,
    },
    5: {
      _id: 5,
      objective: '瞬钓',
      requirement: '达成条件：在幻海流中钓到15条鱼。',
      icon: 65905,
      order: 5,
    },
    6: {
      _id: 6,
      objective: '爆钓王',
      requirement: '达成条件：在幻海流中钓到35条鱼。',
      icon: 65906,
      order: 6,
    },
    7: {
      _id: 7,
      objective: '丰渔：加拉迪翁湾',
      requirement: '达成条件：小队全体在加拉迪翁湾合计钓到10种以上的鱼类。',
      icon: 65907,
      order: 7,
    },
    8: {
      _id: 8,
      objective: '丰渔：梅尔托尔海峡南',
      requirement: '达成条件：小队全体在梅尔托尔海峡南合计钓到10种以上的鱼类。',
      icon: 65908,
      order: 8,
    },
    9: {
      _id: 9,
      objective: '丰渔：梅尔托尔海峡北',
      requirement: '达成条件：小队全体在梅尔托尔海峡北合计钓到10种以上的鱼类。',
      icon: 65909,
      order: 9,
    },
    10: {
      _id: 10,
      objective: '丰渔：罗塔诺海',
      requirement: '达成条件：小队全体在罗塔诺海合计钓到10种以上的鱼类。',
      icon: 65910,
      order: 10,
    },
    11: {
      _id: 11,
      objective: '传说中的大渔旗',
      requirement: '达成条件：小队全体合计钓到3条以上★★★★★的鱼。',
      icon: 65911,
      order: 11,
    },
    12: {
      _id: 12,
      objective: '利姆莱茵的宠爱之子',
      requirement: '达成条件：由自己引发幻海流。',
      icon: 65912,
      order: 12,
    },
    13: {
      _id: 13,
      objective: '八爪旅人',
      requirement: '达成条件：小队全体合计钓到150只以上的章鱼类。',
      icon: 65913,
      order: 13,
    },
    14: {
      _id: 14,
      objective: '捕鲨人',
      requirement: '达成条件：小队全体合计钓到200条以上的鲨鱼类。',
      icon: 65914,
      order: 14,
    },
    15: {
      _id: 15,
      objective: '水母狂魔',
      requirement: '达成条件：小队全体合计钓到150只以上的水母类。',
      icon: 65915,
      order: 15,
    },
    16: {
      _id: 16,
      objective: '龙马惊神',
      requirement: '达成条件：小队全体合计钓到100只以上的海龙或海马。',
      icon: 65916,
      order: 16,
    },
  },
  OCEAN_FISHING_WEATHER: {
    1: {
      _id: 1,
      icon: 60201,
      name_chs: '碧空',
    },
    2: {
      _id: 2,
      icon: 60202,
      name_chs: '晴朗',
    },
    3: {
      _id: 3,
      icon: 60203,
      name_chs: '阴云',
    },
    4: {
      _id: 4,
      icon: 60204,
      name_chs: '薄雾',
    },
    5: {
      _id: 5,
      icon: 60205,
      name_chs: '微风',
    },
    6: {
      _id: 6,
      icon: 60206,
      name_chs: '强风',
    },
    7: {
      _id: 7,
      icon: 60207,
      name_chs: '小雨',
    },
    8: {
      _id: 8,
      icon: 60208,
      name_chs: '暴雨',
    },
    9: {
      _id: 9,
      icon: 60209,
      name_chs: '打雷',
    },
    10: {
      _id: 10,
      icon: 60210,
      name_chs: '雷雨',
    },
    11: {
      _id: 11,
      icon: 60211,
      name_chs: '扬沙',
    },
    14: {
      _id: 14,
      icon: 60214,
      name_chs: '热浪',
    },
    15: {
      _id: 15,
      icon: 60215,
      name_chs: '小雪',
    },
    16: {
      _id: 16,
      icon: 60216,
      name_chs: '暴雪',
    },
  },
  OCEAN_FISHING_TIPS: {
    tip1: {
      title: '[苍海龙骑]海钓手册',
      link: 'https://ngabbs.com/read.php?tid=20553241',
      author: '月咏泠音@摩杜纳',
    },
    tip2: {
      title: '[海钓]300w世界级渔夫达成，个人心得分享。',
      link: 'https://ngabbs.com/read.php?tid=25697469',
      author: '鱼太公@摩杜纳',
      fishTipDict: {
        29788: {
          content: '<div>开局直接火萤。火萤弊端是长中杆鮟鱇咬钩率也会提升，咬钩建议拍掉等轻杆。<br />' +
            '由于天堂和海神印，均有几率在2-3s咬钩，因此3s以下的轻杆建议双提，3s以上必定是海神印，单提。<br />' +
            '1.天堂先咬钩，拍水，只提轻杆。<br />' +
            '2.海神印先咬钩，拍水，双提轻杆。</div>'
        },
        29789: {
          content: '<div>开局用蚕，出直感换小虾肉笼。普通海留以小2cd。如果幻海中，以小2已经cd，建议开耐心1钓HQ以太虱，以小钓大3s内中杆，直接双提赌脸。</div>'
        },
        29790: {
          content: '<div>开局用蚕，出直感后换沟鼠尾巴。深海鳗和浪子鱼咬钩时间极长，通常7-8s左右，钓到后建议拍掉。<br />' +
            '1.沉寂者先咬钩，拍水，换沟鼠尾巴，只提中杆。如果浪子鱼咬钩，继续拍，等长中杆深海鳗。<br />' +
            '2.深海鳗先咬钩，拍水，只提轻杆。如果阿龙纳斯咬钩，继续拍，只提轻杆。</div>'
        },
        29791: {
          content: '<div>开局直接用重铁板钩，随便拍一个，等鱼王杆双提。<br />' +
            '钓法1：出直感时拍掉鳍龙，直感第二杆平提，以取消拍水buff，继续蹲鳍龙。<br />' +
            '钓法2：不拍水，赌第二杆鳍龙咬钩，且起钩时直感已结束。这样可以马上摸第二个直感，钓友自行选择。</div>'
        },
      },
      extra: '<div>关于撒饵：蓝鱼直感期间，我们可能会遇到诸如：7s鮟鱇、7s鳍龙、8.5s深海鳗、8.5s浪子鱼等长时间杂鱼，导致没时间抛出第二杆。为保证15s直感内稳定两杆，可以视GP而定，第一杆加个撒饵。</div>' +
        '<div style="font-weight: bold">直感触发后，记得用个拍水，蓝鱼咬钩率只有0.5%，给这个直感提高质量。</div>',
    }
  },
  SPEAR_FISH: {
    // '17951': {
    //   _id: 17951,
    //   name_chs: '大方士',
    //   spearFishingId: 30002,
    //   locations: [481],
    //   gig: 1,
    //   requiredCnt: 0,
    //   startHour: 0,
    //   endHour: 24,
    //   predators: {
    //     '20228': 7,
    //   },
    //   previousWeatherSet: [],
    //   weatherSet: [],
    //   anglerFishId: null,
    //   patch: 4,
    //   collectable: false,
    // },
    20144: {
      _id: 20144,
      name_chs: '旋螺',
      spearFishingId: 20000,
      locations: [464, 465],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 638,
      patch: 4,
      collectable: false,
    },
    20145: {
      _id: 20145,
      name_chs: '黑箱鲀',
      spearFishingId: 20001,
      locations: [464, 467],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 639,
      patch: 4,
      collectable: false,
    },
    20146: {
      _id: 20146,
      name_chs: '飞鳐',
      spearFishingId: 20002,
      locations: [465],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 640,
      patch: 4,
      collectable: false,
    },
    20147: {
      _id: 20147,
      name_chs: '王鱼',
      spearFishingId: 20003,
      locations: [466, 467],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 641,
      patch: 4,
      collectable: false,
    },
    20148: {
      _id: 20148,
      name_chs: '雪花蛇鳝',
      spearFishingId: 20004,
      locations: [468],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 642,
      patch: 4,
      collectable: false,
    },
    20149: {
      _id: 20149,
      name_chs: '跳虾虎',
      spearFishingId: 20005,
      locations: [469],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 643,
      patch: 4,
      collectable: false,
    },
    20150: {
      _id: 20150,
      name_chs: '光鳞鱼',
      spearFishingId: 20006,
      locations: [469],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 644,
      patch: 4,
      collectable: false,
    },
    20151: {
      _id: 20151,
      name_chs: '星点东方鲀',
      spearFishingId: 20007,
      locations: [470, 471],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 645,
      patch: 4,
      collectable: false,
    },
    20152: {
      _id: 20152,
      name_chs: '花鳗鲡',
      spearFishingId: 20008,
      locations: [470],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 646,
      patch: 4,
      collectable: false,
    },
    20153: {
      _id: 20153,
      name_chs: '寄居虫',
      spearFishingId: 20009,
      locations: [471],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 647,
      patch: 4,
      collectable: false,
    },
    20154: {
      _id: 20154,
      name_chs: '高脚蟹',
      spearFishingId: 20010,
      locations: [464],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 648,
      patch: 4,
      collectable: false,
    },
    20155: {
      _id: 20155,
      name_chs: '海天狗',
      spearFishingId: 20011,
      locations: [465, 466],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 649,
      patch: 4,
      collectable: false,
    },
    20156: {
      _id: 20156,
      name_chs: '黑头巾',
      spearFishingId: 20012,
      locations: [466, 468],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 650,
      patch: 4,
      collectable: false,
    },
    20157: {
      _id: 20157,
      name_chs: '虎斑鲨',
      spearFishingId: 20013,
      locations: [467],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 651,
      patch: 4,
      collectable: false,
    },
    20158: {
      _id: 20158,
      name_chs: '骨螺',
      spearFishingId: 20014,
      locations: [468],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 652,
      patch: 4,
      collectable: false,
    },
    20159: {
      _id: 20159,
      name_chs: '渔鱼',
      spearFishingId: 20015,
      locations: [469],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 653,
      patch: 4,
      collectable: false,
    },
    20160: {
      _id: 20160,
      name_chs: '奥萨德螺',
      spearFishingId: 20016,
      locations: [470],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 654,
      patch: 4,
      collectable: false,
    },
    20161: {
      _id: 20161,
      name_chs: '无须鱼',
      spearFishingId: 20017,
      locations: [471],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 655,
      patch: 4,
      collectable: false,
    },
    20162: {
      _id: 20162,
      name_chs: '烛光鱼',
      spearFishingId: 20018,
      locations: [464],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 656,
      patch: 4,
      collectable: false,
    },
    20163: {
      _id: 20163,
      name_chs: '丝鲹',
      spearFishingId: 20019,
      locations: [464],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 657,
      patch: 4,
      collectable: false,
    },
    20164: {
      _id: 20164,
      name_chs: '狆鳗',
      spearFishingId: 20020,
      locations: [465],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 658,
      patch: 4,
      collectable: false,
    },
    20165: {
      _id: 20165,
      name_chs: '海参',
      spearFishingId: 20021,
      locations: [466, 530],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 659,
      patch: 4,
      collectable: false,
    },
    20166: {
      _id: 20166,
      name_chs: '鞍带石斑鱼',
      spearFishingId: 20022,
      locations: [467],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 660,
      patch: 4,
      collectable: false,
    },
    20167: {
      _id: 20167,
      name_chs: '毒鲉',
      spearFishingId: 20023,
      locations: [468],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 661,
      patch: 4,
      collectable: false,
    },
    20168: {
      _id: 20168,
      name_chs: '赤备',
      spearFishingId: 20024,
      locations: [470],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 662,
      patch: 4,
      collectable: false,
    },
    20169: {
      _id: 20169,
      name_chs: '黑鲢',
      spearFishingId: 20025,
      locations: [470],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 663,
      patch: 4,
      collectable: false,
    },
    20170: {
      _id: 20170,
      name_chs: '古钱贝',
      spearFishingId: 20026,
      locations: [471],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 664,
      patch: 4,
      collectable: false,
    },
    20171: {
      _id: 20171,
      name_chs: '食死鱼',
      spearFishingId: 20027,
      locations: [464, 530],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 665,
      patch: 4,
      collectable: false,
    },
    20172: {
      _id: 20172,
      name_chs: '浪人鲹',
      spearFishingId: 20028,
      locations: [465],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 666,
      patch: 4,
      collectable: false,
    },
    20173: {
      _id: 20173,
      name_chs: '褐石斑鱼',
      spearFishingId: 20029,
      locations: [466],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 667,
      patch: 4,
      collectable: false,
    },
    20174: {
      _id: 20174,
      name_chs: '蝾螺',
      spearFishingId: 20030,
      locations: [467, 480, 530],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 668,
      patch: 4,
      collectable: false,
    },
    20175: {
      _id: 20175,
      name_chs: '红玉海星',
      spearFishingId: 20031,
      locations: [468],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 669,
      patch: 4,
      collectable: false,
    },
    20176: {
      _id: 20176,
      name_chs: '藻屑蟹',
      spearFishingId: 20032,
      locations: [469],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 670,
      patch: 4,
      collectable: false,
    },
    20177: {
      _id: 20177,
      name_chs: '泥仙人',
      spearFishingId: 20033,
      locations: [470],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 671,
      patch: 4,
      collectable: false,
    },
    20178: {
      _id: 20178,
      name_chs: '修仙空棘鱼',
      spearFishingId: 20034,
      locations: [471],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 672,
      patch: 4,
      collectable: false,
    },
    20179: {
      _id: 20179,
      name_chs: '白乌贼',
      spearFishingId: 20035,
      locations: [465, 480],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 673,
      patch: 4,
      collectable: false,
    },
    20180: {
      _id: 20180,
      name_chs: '类鲹',
      spearFishingId: 20036,
      locations: [466],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 674,
      patch: 4,
      collectable: true,
    },
    20181: {
      _id: 20181,
      name_chs: '楚蟹',
      spearFishingId: 20037,
      locations: [467],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 675,
      patch: 4,
      collectable: true,
    },
    20182: {
      _id: 20182,
      name_chs: '红眼鲈',
      spearFishingId: 20038,
      locations: [468],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 676,
      patch: 4,
      collectable: false,
    },
    20183: {
      _id: 20183,
      name_chs: '斜方鱊',
      spearFishingId: 20039,
      locations: [469],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 677,
      patch: 4,
      collectable: false,
    },
    20184: {
      _id: 20184,
      name_chs: '半寿鱼',
      spearFishingId: 20040,
      locations: [469],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 678,
      patch: 4,
      collectable: false,
    },
    20185: {
      _id: 20185,
      name_chs: '蛇头鱼',
      spearFishingId: 20041,
      locations: [471],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 679,
      patch: 4,
      collectable: false,
    },
    20186: {
      _id: 20186,
      name_chs: '红衣',
      spearFishingId: 20042,
      locations: [472],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 680,
      patch: 4,
      collectable: false,
    },
    20187: {
      _id: 20187,
      name_chs: '斑节虾',
      spearFishingId: 20043,
      locations: [473],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 681,
      patch: 4,
      collectable: false,
    },
    20188: {
      _id: 20188,
      name_chs: '天狗团扇',
      spearFishingId: 20044,
      locations: [473],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 682,
      patch: 4,
      collectable: false,
    },
    20189: {
      _id: 20189,
      name_chs: '星轮螺',
      spearFishingId: 20045,
      locations: [474],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 683,
      patch: 4,
      collectable: false,
    },
    20190: {
      _id: 20190,
      name_chs: '青鱼',
      spearFishingId: 20046,
      locations: [474],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 684,
      patch: 4,
      collectable: false,
    },
    20191: {
      _id: 20191,
      name_chs: '菜蛙',
      spearFishingId: 20047,
      locations: [475],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 685,
      patch: 4,
      collectable: false,
    },
    20192: {
      _id: 20192,
      name_chs: '斩马鱼',
      spearFishingId: 20048,
      locations: [475],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 686,
      patch: 4,
      collectable: false,
    },
    20193: {
      _id: 20193,
      name_chs: '红鳍',
      spearFishingId: 20049,
      locations: [476],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 687,
      patch: 4,
      collectable: false,
    },
    20194: {
      _id: 20194,
      name_chs: '月轮',
      spearFishingId: 20050,
      locations: [477],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 688,
      patch: 4,
      collectable: false,
    },
    20195: {
      _id: 20195,
      name_chs: '白骨舌鱼',
      spearFishingId: 20051,
      locations: [477, 478],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 689,
      patch: 4,
      collectable: false,
    },
    20196: {
      _id: 20196,
      name_chs: '盐鲨',
      spearFishingId: 20052,
      locations: [478],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 690,
      patch: 4,
      collectable: false,
    },
    20197: {
      _id: 20197,
      name_chs: '皇家披风',
      spearFishingId: 20053,
      locations: [479],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 691,
      patch: 4,
      collectable: false,
    },
    20198: {
      _id: 20198,
      name_chs: '矶灯笼',
      spearFishingId: 20054,
      locations: [472],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 692,
      patch: 4,
      collectable: false,
    },
    20199: {
      _id: 20199,
      name_chs: '黄尾鰤',
      spearFishingId: 20055,
      locations: [472],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 693,
      patch: 4,
      collectable: false,
    },
    20200: {
      _id: 20200,
      name_chs: '樱鲑',
      spearFishingId: 20056,
      locations: [473],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 694,
      patch: 4,
      collectable: false,
    },
    20201: {
      _id: 20201,
      name_chs: '温泉花蟹',
      spearFishingId: 20057,
      locations: [474],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 695,
      patch: 4,
      collectable: false,
    },
    20202: {
      _id: 20202,
      name_chs: '朵塔儿鮈',
      spearFishingId: 20058,
      locations: [475],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 696,
      patch: 4,
      collectable: false,
    },
    20203: {
      _id: 20203,
      name_chs: '河蚬',
      spearFishingId: 20059,
      locations: [476],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 697,
      patch: 4,
      collectable: false,
    },
    20204: {
      _id: 20204,
      name_chs: '草鲨',
      spearFishingId: 20060,
      locations: [476],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 698,
      patch: 4,
      collectable: false,
    },
    20205: {
      _id: 20205,
      name_chs: '台风虾',
      spearFishingId: 20061,
      locations: [477],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 699,
      patch: 4,
      collectable: false,
    },
    20206: {
      _id: 20206,
      name_chs: '岩牡蛎',
      spearFishingId: 20062,
      locations: [478],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 700,
      patch: 4,
      collectable: false,
    },
    20207: {
      _id: 20207,
      name_chs: '盐湖胆',
      spearFishingId: 20063,
      locations: [479],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 701,
      patch: 4,
      collectable: false,
    },
    20208: {
      _id: 20208,
      name_chs: '刻木蟹',
      spearFishingId: 20064,
      locations: [479],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 702,
      patch: 4,
      collectable: false,
    },
    20209: {
      _id: 20209,
      name_chs: '刺足龙虾',
      spearFishingId: 20065,
      locations: [472, 480, 530],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 703,
      patch: 4,
      collectable: false,
    },
    20210: {
      _id: 20210,
      name_chs: '箕作鲨',
      spearFishingId: 20066,
      locations: [480],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20217: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 704,
      patch: 4,
      collectable: true,
    },
    20211: {
      _id: 20211,
      name_chs: '多玛金鱼',
      spearFishingId: 20067,
      locations: [473, 481],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 705,
      patch: 4,
      collectable: false,
    },
    20212: {
      _id: 20212,
      name_chs: '逆龙',
      spearFishingId: 20068,
      locations: [474, 481],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 706,
      patch: 4,
      collectable: false,
    },
    20213: {
      _id: 20213,
      name_chs: '太阳神使者',
      spearFishingId: 20069,
      locations: [475, 476],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 707,
      patch: 4,
      collectable: false,
    },
    20214: {
      _id: 20214,
      name_chs: '盐螺',
      spearFishingId: 20070,
      locations: [477, 483],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 708,
      patch: 4,
      collectable: false,
    },
    20215: {
      _id: 20215,
      name_chs: '白鲸',
      spearFishingId: 20071,
      locations: [478],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 709,
      patch: 4,
      collectable: false,
    },
    20216: {
      _id: 20216,
      name_chs: '巨型税官',
      spearFishingId: 20072,
      locations: [479],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 710,
      patch: 4,
      collectable: false,
    },
    20217: {
      _id: 20217,
      name_chs: '圆翅燕鱼',
      spearFishingId: 20073,
      locations: [472, 480],
      gig: 1,
      requiredCnt: 7,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 711,
      patch: 4,
      collectable: false,
    },
    20218: {
      _id: 20218,
      name_chs: '河鲷',
      spearFishingId: 20074,
      locations: [473],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 712,
      patch: 4,
      collectable: false,
    },
    20219: {
      _id: 20219,
      name_chs: '绳鱼',
      spearFishingId: 20075,
      locations: [474, 481],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 713,
      patch: 4,
      collectable: false,
    },
    20220: {
      _id: 20220,
      name_chs: '飞天鱼',
      spearFishingId: 20076,
      locations: [481],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20228: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 714,
      patch: 4,
      collectable: true,
    },
    20221: {
      _id: 20221,
      name_chs: '月神使者',
      spearFishingId: 20077,
      locations: [475, 482],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 715,
      patch: 4,
      collectable: false,
    },
    20222: {
      _id: 20222,
      name_chs: '瞪亲刺鱼',
      spearFishingId: 20078,
      locations: [476, 482],
      gig: 3,
      requiredCnt: 7,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 716,
      patch: 4,
      collectable: false,
    },
    20223: {
      _id: 20223,
      name_chs: '阿巴拉提亚皮皮拉鱼',
      spearFishingId: 20079,
      locations: [477, 483],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 717,
      patch: 4,
      collectable: false,
    },
    20224: {
      _id: 20224,
      name_chs: '白钢泥鳅',
      spearFishingId: 20080,
      locations: [478, 483],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 718,
      patch: 4,
      collectable: false,
    },
    20225: {
      _id: 20225,
      name_chs: '象牙鳎',
      spearFishingId: 20081,
      locations: [479, 483],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 719,
      patch: 4,
      collectable: false,
    },
    20226: {
      _id: 20226,
      name_chs: '条石鲷',
      spearFishingId: 20082,
      locations: [472, 530],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 720,
      patch: 4,
      collectable: false,
    },
    20227: {
      _id: 20227,
      name_chs: '千牙龙',
      spearFishingId: 20083,
      locations: [480],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20217: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 721,
      patch: 4,
      collectable: true,
    },
    20228: {
      _id: 20228,
      name_chs: '鱼龙',
      spearFishingId: 20084,
      locations: [473, 481],
      gig: 3,
      requiredCnt: 7,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 722,
      patch: 4,
      collectable: false,
    },
    20229: {
      _id: 20229,
      name_chs: '八角鱼',
      spearFishingId: 20085,
      locations: [474],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 723,
      patch: 4,
      collectable: false,
    },
    20230: {
      _id: 20230,
      name_chs: '方士鱼',
      spearFishingId: 20086,
      locations: [481],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20228: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 724,
      patch: 4,
      collectable: true,
    },
    20231: {
      _id: 20231,
      name_chs: '焰鱼',
      spearFishingId: 20087,
      locations: [475, 482],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 725,
      patch: 4,
      collectable: false,
    },
    20232: {
      _id: 20232,
      name_chs: '不忠蛇',
      spearFishingId: 20088,
      locations: [476, 482],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 726,
      patch: 4,
      collectable: false,
    },
    20233: {
      _id: 20233,
      name_chs: '永恒眼',
      spearFishingId: 20089,
      locations: [482],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20222: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 727,
      patch: 4,
      collectable: true,
    },
    20234: {
      _id: 20234,
      name_chs: '名马魂',
      spearFishingId: 20090,
      locations: [482],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20222: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 728,
      patch: 4,
      collectable: true,
    },
    20235: {
      _id: 20235,
      name_chs: '泛洪金枪鱼',
      spearFishingId: 20091,
      locations: [477],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 729,
      patch: 4,
      collectable: false,
    },
    20236: {
      _id: 20236,
      name_chs: '佣兵蟹',
      spearFishingId: 20092,
      locations: [478],
      gig: 2,
      requiredCnt: 7,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 730,
      patch: 4,
      collectable: false,
    },
    20237: {
      _id: 20237,
      name_chs: '不死鱼',
      spearFishingId: 20093,
      locations: [479],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 731,
      patch: 4,
      collectable: false,
    },
    20238: {
      _id: 20238,
      name_chs: '丝绸翻车鱼',
      spearFishingId: 20094,
      locations: [483],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20236: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 732,
      patch: 4,
      collectable: true,
    },
    20239: {
      _id: 20239,
      name_chs: '沧龙',
      spearFishingId: 20095,
      locations: [483],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20236: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 733,
      patch: 4,
      collectable: true,
    },
    20528: {
      _id: 20528,
      name_chs: '龙之孙',
      spearFishingId: 20096,
      locations: [480],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        20217: 7,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 734,
      patch: 4,
      collectable: false,
    },
    21179: {
      _id: 21179,
      name_chs: '一文字',
      spearFishingId: 20097,
      locations: [472],
      gig: 3,
      requiredCnt: 10,
      startHour: 2,
      endHour: 12,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 740,
      patch: 4.1,
      collectable: false,
    },
    21180: {
      _id: 21180,
      name_chs: '滑溜狮子鱼',
      spearFishingId: 20098,
      locations: [530],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        21179: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 741,
      patch: 4.1,
      collectable: false,
    },
    27516: {
      _id: 27516,
      name_chs: '银灰鲤',
      spearFishingId: 20099,
      locations: [636],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3113,
      patch: 5,
      collectable: false,
    },
    27517: {
      _id: 27517,
      name_chs: '丁香虾虎',
      spearFishingId: 20100,
      locations: [635],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3114,
      patch: 5,
      collectable: false,
    },
    27518: {
      _id: 27518,
      name_chs: '紫鬼鱼',
      spearFishingId: 20101,
      locations: [635, 645],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3115,
      patch: 5,
      collectable: false,
    },
    27519: {
      _id: 27519,
      name_chs: '湖虱',
      spearFishingId: 20108,
      locations: [635, 645],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3116,
      patch: 5,
      collectable: false,
    },
    27520: {
      _id: 27520,
      name_chs: '镜核',
      spearFishingId: 20102,
      locations: [635],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3117,
      patch: 5,
      collectable: false,
    },
    27521: {
      _id: 27521,
      name_chs: '酱紫章鱼',
      spearFishingId: 20109,
      locations: [635],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3118,
      patch: 5,
      collectable: false,
    },
    27522: {
      _id: 27522,
      name_chs: '菁灵枪鱼',
      spearFishingId: 20103,
      locations: [635],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3119,
      patch: 5,
      collectable: false,
    },
    27523: {
      _id: 27523,
      name_chs: '暗影鲤',
      spearFishingId: 20110,
      locations: [636, 645],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3120,
      patch: 5,
      collectable: false,
    },
    27524: {
      _id: 27524,
      name_chs: '湖蓟',
      spearFishingId: 20111,
      locations: [636],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3121,
      patch: 5,
      collectable: false,
    },
    27525: {
      _id: 27525,
      name_chs: '白金鲷鱼',
      spearFishingId: 20112,
      locations: [636, 645],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3122,
      patch: 5,
      collectable: false,
    },
    27526: {
      _id: 27526,
      name_chs: '狱卒鱼',
      spearFishingId: 20116,
      locations: [636],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3123,
      patch: 5,
      collectable: false,
    },
    27527: {
      _id: 27527,
      name_chs: '茄子鱼',
      spearFishingId: 20113,
      locations: [636],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3124,
      patch: 5,
      collectable: false,
    },
    27528: {
      _id: 27528,
      name_chs: '空吻鱼',
      spearFishingId: 20104,
      locations: [637],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3125,
      patch: 5,
      collectable: false,
    },
    27529: {
      _id: 27529,
      name_chs: '堇色蛤蜊',
      spearFishingId: 20105,
      locations: [637],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3126,
      patch: 5,
      collectable: false,
    },
    27530: {
      _id: 27530,
      name_chs: '萎鳃丽鱼',
      spearFishingId: 20114,
      locations: [637],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3127,
      patch: 5,
      collectable: false,
    },
    27531: {
      _id: 27531,
      name_chs: '贵族鱼',
      spearFishingId: 20115,
      locations: [637],
      gig: 2,
      requiredCnt: 10,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3128,
      patch: 5,
      collectable: false,
    },
    27532: {
      _id: 27532,
      name_chs: '雾眼鱼',
      spearFishingId: 20117,
      locations: [637],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3129,
      patch: 5,
      collectable: false,
    },
    27533: {
      _id: 27533,
      name_chs: '雷克兰德鳕鱼',
      spearFishingId: 20106,
      locations: [637],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3130,
      patch: 5,
      collectable: false,
    },
    27534: {
      _id: 27534,
      name_chs: '迷你俾斯麦鱼',
      spearFishingId: 20154,
      locations: [645],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27531: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3131,
      patch: 5,
      collectable: true,
    },
    27535: {
      _id: 27535,
      name_chs: '沟鳞鱼',
      spearFishingId: 20160,
      locations: [645],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27531: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3132,
      patch: 5,
      collectable: true,
    },
    27536: {
      _id: 27536,
      name_chs: '大眼鱼',
      spearFishingId: 20107,
      locations: [642],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3133,
      patch: 5,
      collectable: false,
    },
    27537: {
      _id: 27537,
      name_chs: '杰娜娜的泪珠',
      spearFishingId: 20118,
      locations: [638],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3134,
      patch: 5,
      collectable: false,
    },
    27538: {
      _id: 27538,
      name_chs: '雏菊包头巾',
      spearFishingId: 20120,
      locations: [638],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3135,
      patch: 5,
      collectable: false,
    },
    27539: {
      _id: 27539,
      name_chs: '海蓝钝口螈',
      spearFishingId: 20119,
      locations: [638],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3136,
      patch: 5,
      collectable: false,
    },
    27540: {
      _id: 27540,
      name_chs: '小小爱神',
      spearFishingId: 20121,
      locations: [638],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3137,
      patch: 5,
      collectable: false,
    },
    27541: {
      _id: 27541,
      name_chs: '皮亚雷鱼',
      spearFishingId: 20122,
      locations: [638],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3138,
      patch: 5,
      collectable: false,
    },
    27542: {
      _id: 27542,
      name_chs: '孚布特多鳍鱼',
      spearFishingId: 20126,
      locations: [638],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3139,
      patch: 5,
      collectable: true,
    },
    27543: {
      _id: 27543,
      name_chs: '孔雀鱼',
      spearFishingId: 20127,
      locations: [639],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3140,
      patch: 5,
      collectable: true,
    },
    27544: {
      _id: 27544,
      name_chs: '锔瓷燕鱼',
      spearFishingId: 20123,
      locations: [639],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3141,
      patch: 5,
      collectable: false,
    },
    27545: {
      _id: 27545,
      name_chs: '花芯鱼',
      spearFishingId: 20128,
      locations: [639],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3142,
      patch: 5,
      collectable: false,
    },
    27546: {
      _id: 27546,
      name_chs: '镜像鱼',
      spearFishingId: 20124,
      locations: [639],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3143,
      patch: 5,
      collectable: false,
    },
    27547: {
      _id: 27547,
      name_chs: '玻璃鳗',
      spearFishingId: 20125,
      locations: [639],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3144,
      patch: 5,
      collectable: false,
    },
    27548: {
      _id: 27548,
      name_chs: '孚布特火蜥蜴',
      spearFishingId: 20129,
      locations: [639],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3145,
      patch: 5,
      collectable: false,
    },
    27549: {
      _id: 27549,
      name_chs: '庭园跳鱼',
      spearFishingId: 20130,
      locations: [640],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3146,
      patch: 5,
      collectable: false,
    },
    27550: {
      _id: 27550,
      name_chs: '时髦鱼',
      spearFishingId: 20131,
      locations: [640, 646],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3147,
      patch: 5,
      collectable: false,
    },
    27551: {
      _id: 27551,
      name_chs: '绍尔迪雅红宝石',
      spearFishingId: 20142,
      locations: [640],
      gig: 2,
      requiredCnt: 10,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3148,
      patch: 5,
      collectable: false,
    },
    27552: {
      _id: 27552,
      name_chs: '伪仙子',
      spearFishingId: 20132,
      locations: [640],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3149,
      patch: 5,
      collectable: false,
    },
    27553: {
      _id: 27553,
      name_chs: '猎人矢',
      spearFishingId: 20133,
      locations: [640],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3150,
      patch: 5,
      collectable: false,
    },
    27554: {
      _id: 27554,
      name_chs: '水妖吞噬者',
      spearFishingId: 20134,
      locations: [640],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3151,
      patch: 5,
      collectable: false,
    },
    27555: {
      _id: 27555,
      name_chs: '天堂蟹',
      spearFishingId: 20135,
      locations: [641, 646],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3152,
      patch: 5,
      collectable: false,
    },
    27556: {
      _id: 27556,
      name_chs: '圣法斯里克的怒发',
      spearFishingId: 20136,
      locations: [641],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3153,
      patch: 5,
      collectable: false,
    },
    27557: {
      _id: 27557,
      name_chs: '女王的礼服',
      spearFishingId: 20137,
      locations: [641],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3154,
      patch: 5,
      collectable: false,
    },
    27558: {
      _id: 27558,
      name_chs: '开花凯尔派',
      spearFishingId: 20143,
      locations: [641],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3155,
      patch: 5,
      collectable: false,
    },
    27559: {
      _id: 27559,
      name_chs: '食尸鬼鱼',
      spearFishingId: 20144,
      locations: [641, 646],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3156,
      patch: 5,
      collectable: false,
    },
    27560: {
      _id: 27560,
      name_chs: '柠檬黄鱼',
      spearFishingId: 20145,
      locations: [641],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3157,
      patch: 5,
      collectable: false,
    },
    27561: {
      _id: 27561,
      name_chs: '多恩之角',
      spearFishingId: 20146,
      locations: [642],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3158,
      patch: 5,
      collectable: false,
    },
    27562: {
      _id: 27562,
      name_chs: '水花',
      spearFishingId: 20148,
      locations: [642],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3159,
      patch: 5,
      collectable: false,
    },
    27563: {
      _id: 27563,
      name_chs: '小丑鱼',
      spearFishingId: 20149,
      locations: [642],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3160,
      patch: 5,
      collectable: false,
    },
    27564: {
      _id: 27564,
      name_chs: '蓝电鱼',
      spearFishingId: 20150,
      locations: [642, 646],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3161,
      patch: 5,
      collectable: false,
    },
    27565: {
      _id: 27565,
      name_chs: '秋意落叶',
      spearFishingId: 20151,
      locations: [642],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3162,
      patch: 5,
      collectable: false,
    },
    27566: {
      _id: 27566,
      name_chs: '大琉璃龙睛',
      spearFishingId: 20161,
      locations: [646],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27551: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3163,
      patch: 5,
      collectable: true,
    },
    27567: {
      _id: 27567,
      name_chs: '古代仙子',
      spearFishingId: 20155,
      locations: [646],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27551: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3164,
      patch: 5,
      collectable: true,
    },
    27568: {
      _id: 27568,
      name_chs: '踝陆蟹',
      spearFishingId: 20138,
      locations: [643],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3165,
      patch: 5,
      collectable: false,
    },
    27569: {
      _id: 27569,
      name_chs: '大树之鳞',
      spearFishingId: 20139,
      locations: [643],
      gig: 1,
      requiredCnt: 10,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3166,
      patch: 5,
      collectable: false,
    },
    27570: {
      _id: 27570,
      name_chs: '隆卡琵琶鱼',
      spearFishingId: 20140,
      locations: [643, 696, 697],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3167,
      patch: 5,
      collectable: false,
    },
    27571: {
      _id: 27571,
      name_chs: '暴食蟹',
      spearFishingId: 20141,
      locations: [643, 647],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3168,
      patch: 5,
      collectable: false,
    },
    27572: {
      _id: 27572,
      name_chs: '加托尔的汗水',
      spearFishingId: 20156,
      locations: [643],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3169,
      patch: 5,
      collectable: false,
    },
    27573: {
      _id: 27573,
      name_chs: '钻石骨舌鱼',
      spearFishingId: 20162,
      locations: [643, 647],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3170,
      patch: 5,
      collectable: false,
    },
    27574: {
      _id: 27574,
      name_chs: '隐者风帽',
      spearFishingId: 20152,
      locations: [644, 696, 697],
      gig: 4,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3171,
      patch: 5,
      collectable: false,
    },
    27575: {
      _id: 27575,
      name_chs: '隐者蟹',
      spearFishingId: 20157,
      locations: [644],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3172,
      patch: 5,
      collectable: false,
    },
    27576: {
      _id: 27576,
      name_chs: '巨型水虎鱼',
      spearFishingId: 20153,
      locations: [644, 647],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3173,
      patch: 5,
      collectable: false,
    },
    27577: {
      _id: 27577,
      name_chs: '永暗鲈',
      spearFishingId: 20147,
      locations: [644],
      gig: 2,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3174,
      patch: 5,
      collectable: false,
    },
    27578: {
      _id: 27578,
      name_chs: '洛查特尔巨骨舌鱼',
      spearFishingId: 20158,
      locations: [644],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3175,
      patch: 5,
      collectable: false,
    },
    27579: {
      _id: 27579,
      name_chs: '安帕的使者',
      spearFishingId: 20163,
      locations: [644, 647],
      gig: 3,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {},
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3176,
      patch: 5,
      collectable: false,
    },
    27580: {
      _id: 27580,
      name_chs: '维斯之耳',
      spearFishingId: 20164,
      locations: [647],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27569: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3177,
      patch: 5,
      collectable: true,
    },
    27581: {
      _id: 27581,
      name_chs: '拉凯提卡虾虎',
      spearFishingId: 20159,
      locations: [647],
      gig: 1,
      requiredCnt: 0,
      startHour: 0,
      endHour: 24,
      predators: {
        27569: 10,
      },
      previousWeatherSet: [],
      weatherSet: [],
      anglerFishId: 3178,
      patch: 5,
      collectable: true,
    },
  },
  SPEAR_FISH_GATHERING_POINTS: {
    464: {
      _id: 464,
      placeNameId: 2511,
      name_chs: '红玉炮台附近',
      anglerLocationId: 100151,
      x: 1578,
      y: 1434,
      radius: 800,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    465: {
      _id: 465,
      placeNameId: 2512,
      name_chs: '碧玉水附近',
      anglerLocationId: 100152,
      x: 1344,
      y: 516,
      radius: 600,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    466: {
      _id: 466,
      placeNameId: 2513,
      name_chs: '翠水乡附近',
      anglerLocationId: 100153,
      x: 828,
      y: 964,
      radius: 600,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    467: {
      _id: 467,
      placeNameId: 2514,
      name_chs: '冒险号附近',
      anglerLocationId: 100154,
      x: 768,
      y: 1180,
      radius: 600,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    468: {
      _id: 468,
      placeNameId: 2515,
      name_chs: '紫水宫附近',
      anglerLocationId: 100155,
      x: 192,
      y: 1802,
      radius: 600,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    469: {
      _id: 469,
      placeNameId: 2517,
      name_chs: '无二江底西南部',
      anglerLocationId: 100251,
      x: 704,
      y: 1824,
      radius: 600,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    470: {
      _id: 470,
      placeNameId: 2518,
      name_chs: '无二江底南部',
      anglerLocationId: 100252,
      x: 1022,
      y: 1782,
      radius: 600,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    471: {
      _id: 471,
      placeNameId: 2519,
      name_chs: '高速魔导驱逐艇L-XXIII附近',
      anglerLocationId: 100253,
      x: 1688,
      y: 1776,
      radius: 600,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    472: {
      _id: 472,
      placeNameId: 2516,
      name_chs: '小林丸附近',
      anglerLocationId: 100156,
      x: 1892,
      y: 264,
      radius: 600,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    473: {
      _id: 473,
      placeNameId: 2520,
      name_chs: '沉没江船附近',
      anglerLocationId: 100254,
      x: 444,
      y: 1660,
      radius: 600,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    474: {
      _id: 474,
      placeNameId: 2521,
      name_chs: '大龙瀑水底',
      anglerLocationId: 100255,
      x: 496,
      y: 484,
      radius: 600,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    475: {
      _id: 475,
      placeNameId: 2522,
      name_chs: '太阳湖底西部',
      anglerLocationId: 100351,
      x: 900,
      y: 1100,
      radius: 600,
      territoryPlaceNameId: 2411,
      regionPlaceNameId: 2401,
      map: 372,
      mapFileId: 'e3f3/00',
      size_factor: 100,
    },
    476: {
      _id: 476,
      placeNameId: 2523,
      name_chs: '太阳湖底东部',
      anglerLocationId: 100352,
      x: 1100,
      y: 1100,
      radius: 600,
      territoryPlaceNameId: 2411,
      regionPlaceNameId: 2401,
      map: 372,
      mapFileId: 'e3f3/00',
      size_factor: 100,
    },
    477: {
      _id: 477,
      placeNameId: 2524,
      name_chs: '盐湖底西北部',
      anglerLocationId: 90451,
      x: 644,
      y: 880,
      radius: 600,
      territoryPlaceNameId: 2408,
      regionPlaceNameId: 2400,
      map: 369,
      mapFileId: 'g3f3/00',
      size_factor: 100,
    },
    478: {
      _id: 478,
      placeNameId: 2525,
      name_chs: '盐湖底中央部',
      anglerLocationId: 90452,
      x: 990,
      y: 1100,
      radius: 600,
      territoryPlaceNameId: 2408,
      regionPlaceNameId: 2400,
      map: 369,
      mapFileId: 'g3f3/00',
      size_factor: 100,
    },
    479: {
      _id: 479,
      placeNameId: 2526,
      name_chs: '盐湖底东南部',
      anglerLocationId: 90453,
      x: 1180,
      y: 1350,
      radius: 600,
      territoryPlaceNameId: 2408,
      regionPlaceNameId: 2400,
      map: 369,
      mapFileId: 'g3f3/00',
      size_factor: 100,
    },
    480: {
      _id: 480,
      placeNameId: 3627,
      name_chs: 'The Eastern Ruby Sea',
      anglerLocationId: 100191,
      x: 1961,
      y: 433,
      radius: 300,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    481: {
      _id: 481,
      placeNameId: 3629,
      name_chs: "The Sunken Junk's South Side",
      anglerLocationId: 100291,
      x: 501,
      y: 1839,
      radius: 300,
      territoryPlaceNameId: 2410,
      regionPlaceNameId: 2401,
      map: 354,
      mapFileId: 'e3f2/00',
      size_factor: 100,
    },
    482: {
      _id: 482,
      placeNameId: 3630,
      name_chs: 'Azim Khaat Southern Lakebed',
      anglerLocationId: 100391,
      x: 1034,
      y: 1211,
      radius: 225,
      territoryPlaceNameId: 2411,
      regionPlaceNameId: 2401,
      map: 372,
      mapFileId: 'e3f3/00',
      size_factor: 100,
    },
    483: {
      _id: 483,
      placeNameId: 3631,
      name_chs: 'Loch Seld Deep Lakebed',
      anglerLocationId: 90491,
      x: 1061,
      y: 1194,
      radius: 225,
      territoryPlaceNameId: 2408,
      regionPlaceNameId: 2400,
      map: 369,
      mapFileId: 'g3f3/00',
      size_factor: 100,
    },
    530: {
      _id: 530,
      placeNameId: 3628,
      name_chs: "Kobayashi Maru's Northern Wake",
      anglerLocationId: 100192,
      x: 1911,
      y: 89,
      radius: 300,
      territoryPlaceNameId: 2409,
      regionPlaceNameId: 2401,
      map: 371,
      mapFileId: 'e3f1/00',
      size_factor: 100,
    },
    635: {
      _id: 635,
      placeNameId: 3341,
      name_chs: '始源湖东北',
      anglerLocationId: 120351,
      x: 656,
      y: 1506,
      radius: 800,
      territoryPlaceNameId: 2953,
      regionPlaceNameId: 2950,
      map: 491,
      mapFileId: 'n4f1/00',
      size_factor: 100,
    },
    636: {
      _id: 636,
      placeNameId: 3342,
      name_chs: '贤岛',
      anglerLocationId: 120352,
      x: 324,
      y: 1562,
      radius: 800,
      territoryPlaceNameId: 2953,
      regionPlaceNameId: 2950,
      map: 491,
      mapFileId: 'n4f1/00',
      size_factor: 100,
    },
    637: {
      _id: 637,
      placeNameId: 3343,
      name_chs: '始源湖东南',
      anglerLocationId: 120353,
      x: 646,
      y: 1824,
      radius: 800,
      territoryPlaceNameId: 2953,
      regionPlaceNameId: 2950,
      map: 491,
      mapFileId: 'n4f1/00',
      size_factor: 100,
    },
    638: {
      _id: 638,
      placeNameId: 3344,
      name_chs: '梦羽城北',
      anglerLocationId: 120651,
      x: 968,
      y: 620,
      radius: 600,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    639: {
      _id: 639,
      placeNameId: 3345,
      name_chs: '鱼群镇',
      anglerLocationId: 120652,
      x: 934,
      y: 1046,
      radius: 800,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    640: {
      _id: 640,
      placeNameId: 3346,
      name_chs: '身镜湖中央',
      anglerLocationId: 120653,
      x: 1224,
      y: 1224,
      radius: 800,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    641: {
      _id: 641,
      placeNameId: 3347,
      name_chs: '优雅馆',
      anglerLocationId: 120654,
      x: 1456,
      y: 1404,
      radius: 800,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    642: {
      _id: 642,
      placeNameId: 3348,
      name_chs: '身镜湖南',
      anglerLocationId: 120655,
      x: 1412,
      y: 1784,
      radius: 800,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    643: {
      _id: 643,
      placeNameId: 3349,
      name_chs: '蛇水湖北',
      anglerLocationId: 120751,
      x: 216,
      y: 1234,
      radius: 800,
      territoryPlaceNameId: 2957,
      regionPlaceNameId: 2950,
      map: 495,
      mapFileId: 'n4f5/00',
      size_factor: 100,
    },
    644: {
      _id: 644,
      placeNameId: 3350,
      name_chs: '朵瓦特里沉没神殿',
      anglerLocationId: 120752,
      x: 424,
      y: 1444,
      radius: 800,
      territoryPlaceNameId: 2957,
      regionPlaceNameId: 2950,
      map: 495,
      mapFileId: 'n4f5/00',
      size_factor: 100,
    },
    645: {
      _id: 645,
      placeNameId: 3632,
      name_chs: 'Eastern Isle of Ken',
      anglerLocationId: 120391,
      x: 560,
      y: 1757,
      radius: 300,
      territoryPlaceNameId: 2953,
      regionPlaceNameId: 2950,
      map: 491,
      mapFileId: 'n4f1/00',
      size_factor: 100,
    },
    646: {
      _id: 646,
      placeNameId: 3633,
      name_chs: 'East Longmirror Lake',
      anglerLocationId: 120691,
      x: 1450,
      y: 1183,
      radius: 300,
      territoryPlaceNameId: 2956,
      regionPlaceNameId: 2950,
      map: 494,
      mapFileId: 'n4f4/00',
      size_factor: 100,
    },
    647: {
      _id: 647,
      placeNameId: 3634,
      name_chs: "Deep Lake Tusi Mek'ta",
      anglerLocationId: 120791,
      x: 234,
      y: 1403,
      radius: 300,
      territoryPlaceNameId: 2957,
      regionPlaceNameId: 2950,
      map: 495,
      mapFileId: 'n4f5/00',
      size_factor: 100,
    },
    696: {
      _id: 696,
      placeNameId: 3456,
      name_chs: '蛇水湖中央',
      anglerLocationId: 120753,
      x: 265,
      y: 1335,
      radius: 300,
      territoryPlaceNameId: 2957,
      regionPlaceNameId: 2950,
      map: 495,
      mapFileId: 'n4f5/00',
      size_factor: 100,
    },
    697: {
      _id: 697,
      placeNameId: 3457,
      name_chs: '蛇水湖南',
      anglerLocationId: 120754,
      x: 185,
      y: 1465,
      radius: 300,
      territoryPlaceNameId: 2957,
      regionPlaceNameId: 2950,
      map: 495,
      mapFileId: 'n4f5/00',
      size_factor: 100,
    },
  },
  SPEAR_REGION_TERRITORY_POINT: [
    {
      id: '2400',
      territories: [
        {
          id: '2408',
          spots: [
            {
              id: 477,
              fishList: [20194, 20195, 20205, 20214, 20223, 20235],
              order: 477,
            },
            {
              id: 478,
              fishList: [20196, 20195, 20206, 20215, 20224, 20236],
              order: 478,
            },
            {
              id: 479,
              fishList: [20197, 20207, 20208, 20216, 20225, 20237],
              order: 479,
            },
            {
              id: 483,
              fishList: [20214, 20224, 20223, 20225, 20238, 20239],
              order: 483,
            },
          ],
          order: 477,
        },
      ],
      order: 477,
    },
    {
      id: '2401',
      territories: [
        {
          id: '2409',
          spots: [
            {
              id: 464,
              fishList: [20144, 20145, 20154, 20162, 20163, 20171],
              order: 464,
            },
            {
              id: 465,
              fishList: [20144, 20146, 20155, 20164, 20172, 20179],
              order: 465,
            },
            {
              id: 466,
              fishList: [20147, 20155, 20156, 20165, 20173, 20180],
              order: 466,
            },
            {
              id: 467,
              fishList: [20147, 20145, 20157, 20166, 20174, 20181],
              order: 467,
            },
            {
              id: 468,
              fishList: [20148, 20158, 20156, 20167, 20175, 20182],
              order: 468,
            },
            {
              id: 472,
              fishList: [20186, 20198, 20199, 20209, 20217, 20226, 21179],
              order: 472,
            },
            {
              id: 480,
              fishList: [20174, 20179, 20209, 20217, 20210, 20227, 20528],
              order: 480,
            },
            {
              id: 530,
              fishList: [20165, 20174, 20171, 20209, 20226, 21180],
              order: 530,
            },
          ],
          order: 464,
        },
        {
          id: '2410',
          spots: [
            {
              id: 469,
              fishList: [20149, 20150, 20159, 20176, 20183, 20184],
              order: 469,
            },
            {
              id: 470,
              fishList: [20151, 20152, 20160, 20168, 20169, 20177],
              order: 470,
            },
            {
              id: 471,
              fishList: [20153, 20151, 20161, 20170, 20185, 20178],
              order: 471,
            },
            {
              id: 473,
              fishList: [20187, 20188, 20200, 20211, 20218, 20228],
              order: 473,
            },
            {
              id: 474,
              fishList: [20189, 20190, 20201, 20212, 20219, 20229],
              order: 474,
            },
            {
              id: 481,
              fishList: [20211, 20212, 20219, 20228, 20220, 20230],
              order: 481,
            },
          ],
          order: 469,
        },
        {
          id: '2411',
          spots: [
            {
              id: 475,
              fishList: [20191, 20192, 20202, 20213, 20221, 20231],
              order: 475,
            },
            {
              id: 476,
              fishList: [20193, 20203, 20204, 20213, 20232, 20222],
              order: 476,
            },
            {
              id: 482,
              fishList: [20221, 20222, 20232, 20231, 20233, 20234],
              order: 482,
            },
          ],
          order: 475,
        },
      ],
      order: 464,
    },
    {
      id: '2950',
      territories: [
        {
          id: '2953',
          spots: [
            {
              id: 635,
              fishList: [27517, 27518, 27520, 27522, 27519, 27521],
              order: 635,
            },
            {
              id: 636,
              fishList: [27516, 27523, 27524, 27525, 27527, 27526],
              order: 636,
            },
            {
              id: 637,
              fishList: [27528, 27529, 27533, 27530, 27531, 27532],
              order: 637,
            },
            {
              id: 645,
              fishList: [27518, 27519, 27523, 27525, 27534, 27535],
              order: 645,
            },
          ],
          order: 635,
        },
        {
          id: '2956',
          spots: [
            {
              id: 638,
              fishList: [27537, 27539, 27538, 27540, 27541, 27542],
              order: 638,
            },
            {
              id: 639,
              fishList: [27544, 27546, 27547, 27543, 27545, 27548],
              order: 639,
            },
            {
              id: 640,
              fishList: [27549, 27550, 27552, 27553, 27554, 27551],
              order: 640,
            },
            {
              id: 641,
              fishList: [27555, 27556, 27557, 27558, 27559, 27560],
              order: 641,
            },
            {
              id: 642,
              fishList: [27536, 27561, 27562, 27563, 27564, 27565],
              order: 642,
            },
            {
              id: 646,
              fishList: [27550, 27555, 27559, 27564, 27567, 27566],
              order: 646,
            },
          ],
          order: 638,
        },
        {
          id: '2957',
          spots: [
            {
              id: 643,
              fishList: [27568, 27569, 27570, 27571, 27572, 27573],
              order: 643,
            },
            {
              id: 644,
              fishList: [27577, 27574, 27576, 27575, 27578, 27579],
              order: 644,
            },
            {
              id: 647,
              fishList: [27571, 27576, 27581, 27573, 27579, 27580],
              order: 647,
            },
            {
              id: 696,
              fishList: [27574, 27570],
              order: 696,
            },
            {
              id: 697,
              fishList: [27574, 27570],
              order: 697,
            },
          ],
          order: 643,
        },
      ],
      order: 635,
    },
  ],
  STARLIGHT_CELEBRATION: {
    startTime: 1608616800000,
    endTime: 1609912800000,
    weather: 15,
    spots: [35, 36, 104, 90, 91, 81, 100, 105, 106, 197, 198],
    territories: [129, 128, 133, 132, 339, 340, 341, 641],
    fish: [4905, 7713, 7919, 7923, 7917, 7701],
  },
  TERRITORY_TYPES: {
    1: {
      id: 1,
      placeName: 0,
    },
    2: {
      id: 2,
      placeName: 0,
    },
    3: {
      id: 3,
      placeName: 0,
    },
    4: {
      id: 4,
      placeName: 0,
    },
    5: {
      id: 5,
      placeName: 0,
    },
    6: {
      id: 6,
      placeName: 0,
    },
    7: {
      id: 7,
      placeName: 0,
    },
    8: {
      id: 8,
      placeName: 0,
    },
    9: {
      id: 9,
      placeName: 0,
    },
    10: {
      id: 10,
      placeName: 0,
    },
    11: {
      id: 11,
      placeName: 0,
    },
    13: {
      id: 13,
      placeName: 0,
    },
    14: {
      id: 14,
      placeName: 0,
    },
    15: {
      id: 15,
      placeName: 0,
    },
    16: {
      id: 16,
      placeName: 0,
    },
    17: {
      id: 17,
      placeName: 0,
    },
    18: {
      id: 18,
      placeName: 0,
    },
    19: {
      id: 19,
      placeName: 0,
    },
    20: {
      id: 20,
      placeName: 0,
    },
    21: {
      id: 21,
      placeName: 0,
    },
    22: {
      id: 22,
      placeName: 0,
    },
    24: {
      id: 24,
      placeName: 0,
    },
    25: {
      id: 25,
      placeName: 0,
    },
    26: {
      id: 26,
      placeName: 0,
    },
    27: {
      id: 27,
      placeName: 0,
    },
    28: {
      id: 28,
      placeName: 0,
    },
    29: {
      id: 29,
      placeName: 0,
    },
    128: {
      id: 128,
      placeName: 28,
    },
    129: {
      id: 129,
      placeName: 29,
    },
    130: {
      id: 130,
      placeName: 40,
    },
    131: {
      id: 131,
      placeName: 41,
    },
    132: {
      id: 132,
      placeName: 52,
    },
    133: {
      id: 133,
      placeName: 53,
    },
    134: {
      id: 134,
      placeName: 30,
    },
    135: {
      id: 135,
      placeName: 31,
    },
    136: {
      id: 136,
      placeName: 425,
    },
    137: {
      id: 137,
      placeName: 32,
    },
    138: {
      id: 138,
      placeName: 33,
    },
    139: {
      id: 139,
      placeName: 34,
    },
    140: {
      id: 140,
      placeName: 42,
    },
    141: {
      id: 141,
      placeName: 43,
    },
    142: {
      id: 142,
      placeName: 49,
    },
    143: {
      id: 143,
      placeName: 406,
    },
    144: {
      id: 144,
      placeName: 1484,
    },
    145: {
      id: 145,
      placeName: 44,
    },
    146: {
      id: 146,
      placeName: 45,
    },
    147: {
      id: 147,
      placeName: 46,
    },
    148: {
      id: 148,
      placeName: 54,
    },
    149: {
      id: 149,
      placeName: 1664,
    },
    150: {
      id: 150,
      placeName: 418,
    },
    151: {
      id: 151,
      placeName: 1431,
    },
    152: {
      id: 152,
      placeName: 55,
    },
    153: {
      id: 153,
      placeName: 56,
    },
    154: {
      id: 154,
      placeName: 57,
    },
    155: {
      id: 155,
      placeName: 63,
    },
    156: {
      id: 156,
      placeName: 67,
    },
    157: {
      id: 157,
      placeName: 35,
    },
    158: {
      id: 158,
      placeName: 36,
    },
    159: {
      id: 159,
      placeName: 37,
    },
    160: {
      id: 160,
      placeName: 230,
    },
    161: {
      id: 161,
      placeName: 48,
    },
    162: {
      id: 162,
      placeName: 49,
    },
    163: {
      id: 163,
      placeName: 50,
    },
    164: {
      id: 164,
      placeName: 58,
    },
    166: {
      id: 166,
      placeName: 59,
    },
    167: {
      id: 167,
      placeName: 128,
    },
    168: {
      id: 168,
      placeName: 401,
    },
    169: {
      id: 169,
      placeName: 61,
    },
    170: {
      id: 170,
      placeName: 47,
    },
    171: {
      id: 171,
      placeName: 64,
    },
    172: {
      id: 172,
      placeName: 65,
    },
    174: {
      id: 174,
      placeName: 478,
    },
    175: {
      id: 175,
      placeName: 0,
    },
    176: {
      id: 176,
      placeName: 153,
    },
    177: {
      id: 177,
      placeName: 733,
    },
    178: {
      id: 178,
      placeName: 617,
    },
    179: {
      id: 179,
      placeName: 548,
    },
    180: {
      id: 180,
      placeName: 350,
    },
    181: {
      id: 181,
      placeName: 27,
    },
    182: {
      id: 182,
      placeName: 40,
    },
    183: {
      id: 183,
      placeName: 52,
    },
    184: {
      id: 184,
      placeName: 0,
    },
    185: {
      id: 185,
      placeName: 0,
    },
    186: {
      id: 186,
      placeName: 0,
    },
    187: {
      id: 187,
      placeName: 0,
    },
    188: {
      id: 188,
      placeName: 37,
    },
    189: {
      id: 189,
      placeName: 128,
    },
    190: {
      id: 190,
      placeName: 54,
    },
    191: {
      id: 191,
      placeName: 55,
    },
    192: {
      id: 192,
      placeName: 56,
    },
    193: {
      id: 193,
      placeName: 1406,
    },
    194: {
      id: 194,
      placeName: 1407,
    },
    195: {
      id: 195,
      placeName: 1408,
    },
    196: {
      id: 196,
      placeName: 1409,
    },
    197: {
      id: 197,
      placeName: 0,
    },
    198: {
      id: 198,
      placeName: 351,
    },
    199: {
      id: 199,
      placeName: 0,
    },
    200: {
      id: 200,
      placeName: 0,
    },
    201: {
      id: 201,
      placeName: 0,
    },
    202: {
      id: 202,
      placeName: 357,
    },
    203: {
      id: 203,
      placeName: 0,
    },
    204: {
      id: 204,
      placeName: 346,
    },
    205: {
      id: 205,
      placeName: 347,
    },
    206: {
      id: 206,
      placeName: 359,
    },
    207: {
      id: 207,
      placeName: 360,
    },
    208: {
      id: 208,
      placeName: 361,
    },
    209: {
      id: 209,
      placeName: 0,
    },
    210: {
      id: 210,
      placeName: 354,
    },
    211: {
      id: 211,
      placeName: 0,
    },
    212: {
      id: 212,
      placeName: 356,
    },
    213: {
      id: 213,
      placeName: 0,
    },
    214: {
      id: 214,
      placeName: 30,
    },
    215: {
      id: 215,
      placeName: 42,
    },
    216: {
      id: 216,
      placeName: 43,
    },
    217: {
      id: 217,
      placeName: 260,
    },
    218: {
      id: 218,
      placeName: 0,
    },
    219: {
      id: 219,
      placeName: 54,
    },
    220: {
      id: 220,
      placeName: 56,
    },
    221: {
      id: 221,
      placeName: 34,
    },
    222: {
      id: 222,
      placeName: 31,
    },
    223: {
      id: 223,
      placeName: 63,
    },
    224: {
      id: 224,
      placeName: 430,
    },
    225: {
      id: 225,
      placeName: 54,
    },
    226: {
      id: 226,
      placeName: 54,
    },
    227: {
      id: 227,
      placeName: 54,
    },
    228: {
      id: 228,
      placeName: 57,
    },
    229: {
      id: 229,
      placeName: 56,
    },
    230: {
      id: 230,
      placeName: 54,
    },
    231: {
      id: 231,
      placeName: 56,
    },
    232: {
      id: 232,
      placeName: 56,
    },
    233: {
      id: 233,
      placeName: 54,
    },
    234: {
      id: 234,
      placeName: 55,
    },
    235: {
      id: 235,
      placeName: 56,
    },
    236: {
      id: 236,
      placeName: 56,
    },
    237: {
      id: 237,
      placeName: 54,
    },
    238: {
      id: 238,
      placeName: 53,
    },
    239: {
      id: 239,
      placeName: 54,
    },
    240: {
      id: 240,
      placeName: 57,
    },
    241: {
      id: 241,
      placeName: 464,
    },
    242: {
      id: 242,
      placeName: 465,
    },
    243: {
      id: 243,
      placeName: 466,
    },
    244: {
      id: 244,
      placeName: 467,
    },
    245: {
      id: 245,
      placeName: 468,
    },
    246: {
      id: 246,
      placeName: 1410,
    },
    247: {
      id: 247,
      placeName: 469,
    },
    248: {
      id: 248,
      placeName: 43,
    },
    249: {
      id: 249,
      placeName: 31,
    },
    250: {
      id: 250,
      placeName: 358,
    },
    251: {
      id: 251,
      placeName: 40,
    },
    252: {
      id: 252,
      placeName: 30,
    },
    253: {
      id: 253,
      placeName: 43,
    },
    254: {
      id: 254,
      placeName: 40,
    },
    255: {
      id: 255,
      placeName: 42,
    },
    256: {
      id: 256,
      placeName: 44,
    },
    257: {
      id: 257,
      placeName: 44,
    },
    258: {
      id: 258,
      placeName: 43,
    },
    259: {
      id: 259,
      placeName: 40,
    },
    260: {
      id: 260,
      placeName: 45,
    },
    261: {
      id: 261,
      placeName: 45,
    },
    262: {
      id: 262,
      placeName: 31,
    },
    263: {
      id: 263,
      placeName: 33,
    },
    264: {
      id: 264,
      placeName: 31,
    },
    265: {
      id: 265,
      placeName: 31,
    },
    266: {
      id: 266,
      placeName: 44,
    },
    267: {
      id: 267,
      placeName: 42,
    },
    268: {
      id: 268,
      placeName: 44,
    },
    269: {
      id: 269,
      placeName: 42,
    },
    270: {
      id: 270,
      placeName: 43,
    },
    271: {
      id: 271,
      placeName: 43,
    },
    272: {
      id: 272,
      placeName: 30,
    },
    273: {
      id: 273,
      placeName: 42,
    },
    274: {
      id: 274,
      placeName: 40,
    },
    275: {
      id: 275,
      placeName: 44,
    },
    276: {
      id: 276,
      placeName: 379,
    },
    277: {
      id: 277,
      placeName: 55,
    },
    278: {
      id: 278,
      placeName: 42,
    },
    279: {
      id: 279,
      placeName: 31,
    },
    280: {
      id: 280,
      placeName: 33,
    },
    281: {
      id: 281,
      placeName: 1334,
    },
    282: {
      id: 282,
      placeName: 1100,
    },
    283: {
      id: 283,
      placeName: 1101,
    },
    284: {
      id: 284,
      placeName: 1102,
    },
    285: {
      id: 285,
      placeName: 30,
    },
    286: {
      id: 286,
      placeName: 462,
    },
    287: {
      id: 287,
      placeName: 31,
    },
    288: {
      id: 288,
      placeName: 462,
    },
    289: {
      id: 289,
      placeName: 55,
    },
    290: {
      id: 290,
      placeName: 55,
    },
    291: {
      id: 291,
      placeName: 56,
    },
    292: {
      id: 292,
      placeName: 357,
    },
    293: {
      id: 293,
      placeName: 359,
    },
    294: {
      id: 294,
      placeName: 361,
    },
    295: {
      id: 295,
      placeName: 357,
    },
    296: {
      id: 296,
      placeName: 359,
    },
    297: {
      id: 297,
      placeName: 361,
    },
    298: {
      id: 298,
      placeName: 63,
    },
    299: {
      id: 299,
      placeName: 67,
    },
    300: {
      id: 300,
      placeName: 67,
    },
    301: {
      id: 301,
      placeName: 63,
    },
    302: {
      id: 302,
      placeName: 63,
    },
    303: {
      id: 303,
      placeName: 55,
    },
    304: {
      id: 304,
      placeName: 63,
    },
    305: {
      id: 305,
      placeName: 67,
    },
    306: {
      id: 306,
      placeName: 45,
    },
    307: {
      id: 307,
      placeName: 31,
    },
    308: {
      id: 308,
      placeName: 67,
    },
    309: {
      id: 309,
      placeName: 67,
    },
    310: {
      id: 310,
      placeName: 32,
    },
    311: {
      id: 311,
      placeName: 32,
    },
    312: {
      id: 312,
      placeName: 45,
    },
    313: {
      id: 313,
      placeName: 63,
    },
    314: {
      id: 314,
      placeName: 43,
    },
    315: {
      id: 315,
      placeName: 67,
    },
    316: {
      id: 316,
      placeName: 63,
    },
    317: {
      id: 317,
      placeName: 56,
    },
    318: {
      id: 318,
      placeName: 45,
    },
    319: {
      id: 319,
      placeName: 54,
    },
    320: {
      id: 320,
      placeName: 54,
    },
    321: {
      id: 321,
      placeName: 57,
    },
    322: {
      id: 322,
      placeName: 63,
    },
    323: {
      id: 323,
      placeName: 45,
    },
    324: {
      id: 324,
      placeName: 57,
    },
    325: {
      id: 325,
      placeName: 350,
    },
    326: {
      id: 326,
      placeName: 67,
    },
    327: {
      id: 327,
      placeName: 32,
    },
    328: {
      id: 328,
      placeName: 34,
    },
    329: {
      id: 329,
      placeName: 37,
    },
    330: {
      id: 330,
      placeName: 33,
    },
    331: {
      id: 331,
      placeName: 459,
    },
    332: {
      id: 332,
      placeName: 42,
    },
    333: {
      id: 333,
      placeName: 0,
    },
    334: {
      id: 334,
      placeName: 0,
    },
    335: {
      id: 335,
      placeName: 67,
    },
    336: {
      id: 336,
      placeName: 0,
    },
    337: {
      id: 337,
      placeName: 0,
    },
    338: {
      id: 338,
      placeName: 152,
    },
    339: {
      id: 339,
      placeName: 425,
    },
    340: {
      id: 340,
      placeName: 426,
    },
    341: {
      id: 341,
      placeName: 427,
    },
    342: {
      id: 342,
      placeName: 1106,
    },
    343: {
      id: 343,
      placeName: 1107,
    },
    344: {
      id: 344,
      placeName: 1108,
    },
    345: {
      id: 345,
      placeName: 1103,
    },
    346: {
      id: 346,
      placeName: 1104,
    },
    347: {
      id: 347,
      placeName: 1105,
    },
    348: {
      id: 348,
      placeName: 477,
    },
    349: {
      id: 349,
      placeName: 48,
    },
    350: {
      id: 350,
      placeName: 59,
    },
    351: {
      id: 351,
      placeName: 481,
    },
    352: {
      id: 352,
      placeName: 0,
    },
    353: {
      id: 353,
      placeName: 2499,
    },
    354: {
      id: 354,
      placeName: 3218,
    },
    355: {
      id: 355,
      placeName: 1301,
    },
    356: {
      id: 356,
      placeName: 1302,
    },
    357: {
      id: 357,
      placeName: 1303,
    },
    358: {
      id: 358,
      placeName: 1304,
    },
    359: {
      id: 359,
      placeName: 1334,
    },
    360: {
      id: 360,
      placeName: 49,
    },
    361: {
      id: 361,
      placeName: 1377,
    },
    362: {
      id: 362,
      placeName: 36,
    },
    363: {
      id: 363,
      placeName: 125,
    },
    364: {
      id: 364,
      placeName: 360,
    },
    365: {
      id: 365,
      placeName: 401,
    },
    366: {
      id: 366,
      placeName: 386,
    },
    367: {
      id: 367,
      placeName: 50,
    },
    368: {
      id: 368,
      placeName: 392,
    },
    369: {
      id: 369,
      placeName: 694,
    },
    370: {
      id: 370,
      placeName: 1305,
    },
    371: {
      id: 371,
      placeName: 404,
    },
    372: {
      id: 372,
      placeName: 493,
    },
    373: {
      id: 373,
      placeName: 58,
    },
    374: {
      id: 374,
      placeName: 1363,
    },
    375: {
      id: 375,
      placeName: 1363,
    },
    376: {
      id: 376,
      placeName: 1374,
    },
    377: {
      id: 377,
      placeName: 1399,
    },
    378: {
      id: 378,
      placeName: 1399,
    },
    379: {
      id: 379,
      placeName: 67,
    },
    380: {
      id: 380,
      placeName: 1301,
    },
    381: {
      id: 381,
      placeName: 1302,
    },
    382: {
      id: 382,
      placeName: 1303,
    },
    383: {
      id: 383,
      placeName: 1304,
    },
    384: {
      id: 384,
      placeName: 1157,
    },
    385: {
      id: 385,
      placeName: 1159,
    },
    386: {
      id: 386,
      placeName: 1158,
    },
    387: {
      id: 387,
      placeName: 35,
    },
    388: {
      id: 388,
      placeName: 1500,
    },
    389: {
      id: 389,
      placeName: 1500,
    },
    390: {
      id: 390,
      placeName: 1500,
    },
    391: {
      id: 391,
      placeName: 1500,
    },
    392: {
      id: 392,
      placeName: 112,
    },
    393: {
      id: 393,
      placeName: 112,
    },
    394: {
      id: 394,
      placeName: 56,
    },
    395: {
      id: 395,
      placeName: 1429,
    },
    396: {
      id: 396,
      placeName: 128,
    },
    397: {
      id: 397,
      placeName: 2200,
    },
    398: {
      id: 398,
      placeName: 2000,
    },
    399: {
      id: 399,
      placeName: 2001,
    },
    400: {
      id: 400,
      placeName: 2002,
    },
    401: {
      id: 401,
      placeName: 2100,
    },
    402: {
      id: 402,
      placeName: 2101,
    },
    403: {
      id: 403,
      placeName: 2691,
    },
    404: {
      id: 404,
      placeName: 29,
    },
    405: {
      id: 405,
      placeName: 33,
    },
    406: {
      id: 406,
      placeName: 33,
    },
    407: {
      id: 407,
      placeName: 462,
    },
    408: {
      id: 408,
      placeName: 32,
    },
    409: {
      id: 409,
      placeName: 28,
    },
    410: {
      id: 410,
      placeName: 46,
    },
    411: {
      id: 411,
      placeName: 32,
    },
    412: {
      id: 412,
      placeName: 34,
    },
    413: {
      id: 413,
      placeName: 33,
    },
    414: {
      id: 414,
      placeName: 32,
    },
    415: {
      id: 415,
      placeName: 31,
    },
    416: {
      id: 416,
      placeName: 2038,
    },
    417: {
      id: 417,
      placeName: 1500,
    },
    418: {
      id: 418,
      placeName: 2300,
    },
    419: {
      id: 419,
      placeName: 2301,
    },
    420: {
      id: 420,
      placeName: 2130,
    },
    421: {
      id: 421,
      placeName: 2327,
    },
    422: {
      id: 422,
      placeName: 0,
    },
    423: {
      id: 423,
      placeName: 1227,
    },
    424: {
      id: 424,
      placeName: 1228,
    },
    425: {
      id: 425,
      placeName: 1229,
    },
    426: {
      id: 426,
      placeName: 1390,
    },
    427: {
      id: 427,
      placeName: 2337,
    },
    428: {
      id: 428,
      placeName: 2335,
    },
    429: {
      id: 429,
      placeName: 2310,
    },
    430: {
      id: 430,
      placeName: 2148,
    },
    431: {
      id: 431,
      placeName: 496,
    },
    432: {
      id: 432,
      placeName: 2081,
    },
    433: {
      id: 433,
      placeName: 2320,
    },
    434: {
      id: 434,
      placeName: 2214,
    },
    435: {
      id: 435,
      placeName: 2050,
    },
    436: {
      id: 436,
      placeName: 2151,
    },
    437: {
      id: 437,
      placeName: 2178,
    },
    438: {
      id: 438,
      placeName: 2147,
    },
    439: {
      id: 439,
      placeName: 2313,
    },
    440: {
      id: 440,
      placeName: 2336,
    },
    441: {
      id: 441,
      placeName: 2031,
    },
    442: {
      id: 442,
      placeName: 1628,
    },
    443: {
      id: 443,
      placeName: 1633,
    },
    444: {
      id: 444,
      placeName: 1638,
    },
    445: {
      id: 445,
      placeName: 1645,
    },
    446: {
      id: 446,
      placeName: 2081,
    },
    447: {
      id: 447,
      placeName: 2151,
    },
    448: {
      id: 448,
      placeName: 2178,
    },
    449: {
      id: 449,
      placeName: 1628,
    },
    450: {
      id: 450,
      placeName: 1633,
    },
    451: {
      id: 451,
      placeName: 1638,
    },
    452: {
      id: 452,
      placeName: 1645,
    },
    453: {
      id: 453,
      placeName: 33,
    },
    454: {
      id: 454,
      placeName: 34,
    },
    455: {
      id: 455,
      placeName: 2100,
    },
    456: {
      id: 456,
      placeName: 2336,
    },
    457: {
      id: 457,
      placeName: 1399,
    },
    458: {
      id: 458,
      placeName: 2300,
    },
    459: {
      id: 459,
      placeName: 2101,
    },
    460: {
      id: 460,
      placeName: 49,
    },
    461: {
      id: 461,
      placeName: 2100,
    },
    462: {
      id: 462,
      placeName: 2083,
    },
    463: {
      id: 463,
      placeName: 2036,
    },
    464: {
      id: 464,
      placeName: 2000,
    },
    465: {
      id: 465,
      placeName: 44,
    },
    466: {
      id: 466,
      placeName: 34,
    },
    467: {
      id: 467,
      placeName: 2200,
    },
    468: {
      id: 468,
      placeName: 63,
    },
    469: {
      id: 469,
      placeName: 63,
    },
    470: {
      id: 470,
      placeName: 2200,
    },
    471: {
      id: 471,
      placeName: 32,
    },
    472: {
      id: 472,
      placeName: 2200,
    },
    473: {
      id: 473,
      placeName: 56,
    },
    474: {
      id: 474,
      placeName: 28,
    },
    475: {
      id: 475,
      placeName: 63,
    },
    476: {
      id: 476,
      placeName: 2001,
    },
    477: {
      id: 477,
      placeName: 2200,
    },
    478: {
      id: 478,
      placeName: 2082,
    },
    479: {
      id: 479,
      placeName: 2200,
    },
    480: {
      id: 480,
      placeName: 67,
    },
    481: {
      id: 481,
      placeName: 2000,
    },
    482: {
      id: 482,
      placeName: 2000,
    },
    483: {
      id: 483,
      placeName: 46,
    },
    484: {
      id: 484,
      placeName: 31,
    },
    485: {
      id: 485,
      placeName: 2001,
    },
    486: {
      id: 486,
      placeName: 350,
    },
    487: {
      id: 487,
      placeName: 63,
    },
    488: {
      id: 488,
      placeName: 63,
    },
    489: {
      id: 489,
      placeName: 2200,
    },
    490: {
      id: 490,
      placeName: 1377,
    },
    491: {
      id: 491,
      placeName: 45,
    },
    492: {
      id: 492,
      placeName: 2100,
    },
    493: {
      id: 493,
      placeName: 2200,
    },
    494: {
      id: 494,
      placeName: 44,
    },
    495: {
      id: 495,
      placeName: 31,
    },
    496: {
      id: 496,
      placeName: 63,
    },
    497: {
      id: 497,
      placeName: 2200,
    },
    498: {
      id: 498,
      placeName: 2200,
    },
    499: {
      id: 499,
      placeName: 2301,
    },
    500: {
      id: 500,
      placeName: 63,
    },
    501: {
      id: 501,
      placeName: 2002,
    },
    502: {
      id: 502,
      placeName: 1374,
    },
    503: {
      id: 503,
      placeName: 2001,
    },
    504: {
      id: 504,
      placeName: 1660,
    },
    505: {
      id: 505,
      placeName: 2041,
    },
    506: {
      id: 506,
      placeName: 1500,
    },
    507: {
      id: 507,
      placeName: 2179,
    },
    508: {
      id: 508,
      placeName: 2181,
    },
    509: {
      id: 509,
      placeName: 359,
    },
    510: {
      id: 510,
      placeName: 230,
    },
    511: {
      id: 511,
      placeName: 2034,
    },
    512: {
      id: 512,
      placeName: 1647,
    },
    513: {
      id: 513,
      placeName: 2327,
    },
    514: {
      id: 514,
      placeName: 1647,
    },
    515: {
      id: 515,
      placeName: 1647,
    },
    516: {
      id: 516,
      placeName: 2088,
    },
    517: {
      id: 517,
      placeName: 2256,
    },
    518: {
      id: 518,
      placeName: 0,
    },
    519: {
      id: 519,
      placeName: 125,
    },
    520: {
      id: 520,
      placeName: 1708,
    },
    521: {
      id: 521,
      placeName: 1714,
    },
    522: {
      id: 522,
      placeName: 1723,
    },
    523: {
      id: 523,
      placeName: 1731,
    },
    524: {
      id: 524,
      placeName: 2256,
    },
    525: {
      id: 525,
      placeName: 1664,
    },
    526: {
      id: 526,
      placeName: 0,
    },
    527: {
      id: 527,
      placeName: 1664,
    },
    528: {
      id: 528,
      placeName: 0,
    },
    529: {
      id: 529,
      placeName: 1708,
    },
    530: {
      id: 530,
      placeName: 1714,
    },
    531: {
      id: 531,
      placeName: 1723,
    },
    532: {
      id: 532,
      placeName: 1731,
    },
    533: {
      id: 533,
      placeName: 63,
    },
    534: {
      id: 534,
      placeName: 1800,
    },
    535: {
      id: 535,
      placeName: 1801,
    },
    536: {
      id: 536,
      placeName: 1802,
    },
    537: {
      id: 537,
      placeName: 154,
    },
    538: {
      id: 538,
      placeName: 154,
    },
    539: {
      id: 539,
      placeName: 154,
    },
    540: {
      id: 540,
      placeName: 154,
    },
    541: {
      id: 541,
      placeName: 154,
    },
    542: {
      id: 542,
      placeName: 154,
    },
    543: {
      id: 543,
      placeName: 154,
    },
    544: {
      id: 544,
      placeName: 154,
    },
    545: {
      id: 545,
      placeName: 154,
    },
    546: {
      id: 546,
      placeName: 154,
    },
    547: {
      id: 547,
      placeName: 154,
    },
    548: {
      id: 548,
      placeName: 154,
    },
    549: {
      id: 549,
      placeName: 154,
    },
    550: {
      id: 550,
      placeName: 154,
    },
    551: {
      id: 551,
      placeName: 154,
    },
    552: {
      id: 552,
      placeName: 33,
    },
    553: {
      id: 553,
      placeName: 2041,
    },
    554: {
      id: 554,
      placeName: 1740,
    },
    555: {
      id: 555,
      placeName: 2090,
    },
    556: {
      id: 556,
      placeName: 1742,
    },
    557: {
      id: 557,
      placeName: 1377,
    },
    558: {
      id: 558,
      placeName: 1799,
    },
    559: {
      id: 559,
      placeName: 406,
    },
    560: {
      id: 560,
      placeName: 2147,
    },
    561: {
      id: 561,
      placeName: 1793,
    },
    562: {
      id: 562,
      placeName: 1793,
    },
    563: {
      id: 563,
      placeName: 1793,
    },
    564: {
      id: 564,
      placeName: 1793,
    },
    565: {
      id: 565,
      placeName: 1793,
    },
    566: {
      id: 566,
      placeName: 406,
    },
    567: {
      id: 567,
      placeName: 1803,
    },
    568: {
      id: 568,
      placeName: 1804,
    },
    569: {
      id: 569,
      placeName: 406,
    },
    570: {
      id: 570,
      placeName: 1793,
    },
    571: {
      id: 571,
      placeName: 1834,
    },
    572: {
      id: 572,
      placeName: 1792,
    },
    573: {
      id: 573,
      placeName: 1811,
    },
    574: {
      id: 574,
      placeName: 1813,
    },
    575: {
      id: 575,
      placeName: 1815,
    },
    576: {
      id: 576,
      placeName: 2265,
    },
    577: {
      id: 577,
      placeName: 2265,
    },
    578: {
      id: 578,
      placeName: 2038,
    },
    579: {
      id: 579,
      placeName: 1665,
    },
    580: {
      id: 580,
      placeName: 1835,
    },
    581: {
      id: 581,
      placeName: 1841,
    },
    582: {
      id: 582,
      placeName: 1847,
    },
    583: {
      id: 583,
      placeName: 1853,
    },
    584: {
      id: 584,
      placeName: 1835,
    },
    585: {
      id: 585,
      placeName: 1841,
    },
    586: {
      id: 586,
      placeName: 1847,
    },
    587: {
      id: 587,
      placeName: 1853,
    },
    588: {
      id: 588,
      placeName: 1847,
    },
    589: {
      id: 589,
      placeName: 1500,
    },
    590: {
      id: 590,
      placeName: 1500,
    },
    591: {
      id: 591,
      placeName: 1500,
    },
    592: {
      id: 592,
      placeName: 357,
    },
    593: {
      id: 593,
      placeName: 1793,
    },
    594: {
      id: 594,
      placeName: 1793,
    },
    595: {
      id: 595,
      placeName: 1793,
    },
    596: {
      id: 596,
      placeName: 1793,
    },
    597: {
      id: 597,
      placeName: 1793,
    },
    598: {
      id: 598,
      placeName: 1793,
    },
    599: {
      id: 599,
      placeName: 1793,
    },
    600: {
      id: 600,
      placeName: 1793,
    },
    601: {
      id: 601,
      placeName: 1793,
    },
    602: {
      id: 602,
      placeName: 1793,
    },
    603: {
      id: 603,
      placeName: 1793,
    },
    604: {
      id: 604,
      placeName: 1793,
    },
    605: {
      id: 605,
      placeName: 1793,
    },
    606: {
      id: 606,
      placeName: 1793,
    },
    607: {
      id: 607,
      placeName: 1793,
    },
    608: {
      id: 608,
      placeName: 1812,
    },
    609: {
      id: 609,
      placeName: 1814,
    },
    610: {
      id: 610,
      placeName: 1816,
    },
    611: {
      id: 611,
      placeName: 695,
    },
    612: {
      id: 612,
      placeName: 2406,
    },
    613: {
      id: 613,
      placeName: 2409,
    },
    614: {
      id: 614,
      placeName: 2410,
    },
    615: {
      id: 615,
      placeName: 1857,
    },
    616: {
      id: 616,
      placeName: 2779,
    },
    617: {
      id: 617,
      placeName: 2031,
    },
    618: {
      id: 618,
      placeName: 0,
    },
    619: {
      id: 619,
      placeName: 1664,
    },
    620: {
      id: 620,
      placeName: 2407,
    },
    621: {
      id: 621,
      placeName: 2408,
    },
    622: {
      id: 622,
      placeName: 2411,
    },
    623: {
      id: 623,
      placeName: 2833,
    },
    624: {
      id: 624,
      placeName: 1647,
    },
    625: {
      id: 625,
      placeName: 1647,
    },
    626: {
      id: 626,
      placeName: 2297,
    },
    627: {
      id: 627,
      placeName: 1868,
    },
    628: {
      id: 628,
      placeName: 2404,
    },
    629: {
      id: 629,
      placeName: 2413,
    },
    630: {
      id: 630,
      placeName: 1647,
    },
    631: {
      id: 631,
      placeName: 0,
    },
    632: {
      id: 632,
      placeName: 1884,
    },
    633: {
      id: 633,
      placeName: 1374,
    },
    634: {
      id: 634,
      placeName: 2410,
    },
    635: {
      id: 635,
      placeName: 2403,
    },
    636: {
      id: 636,
      placeName: 1887,
    },
    637: {
      id: 637,
      placeName: 2266,
    },
    638: {
      id: 638,
      placeName: 2266,
    },
    639: {
      id: 639,
      placeName: 2927,
    },
    640: {
      id: 640,
      placeName: 2406,
    },
    641: {
      id: 641,
      placeName: 2412,
    },
    642: {
      id: 642,
      placeName: 0,
    },
    643: {
      id: 643,
      placeName: 0,
    },
    644: {
      id: 644,
      placeName: 1884,
    },
    645: {
      id: 645,
      placeName: 0,
    },
    646: {
      id: 646,
      placeName: 1884,
    },
    647: {
      id: 647,
      placeName: 2406,
    },
    648: {
      id: 648,
      placeName: 2406,
    },
    649: {
      id: 649,
      placeName: 1893,
    },
    650: {
      id: 650,
      placeName: 1894,
    },
    651: {
      id: 651,
      placeName: 1895,
    },
    652: {
      id: 652,
      placeName: 2270,
    },
    653: {
      id: 653,
      placeName: 2271,
    },
    654: {
      id: 654,
      placeName: 2272,
    },
    655: {
      id: 655,
      placeName: 2273,
    },
    656: {
      id: 656,
      placeName: 1647,
    },
    657: {
      id: 657,
      placeName: 2409,
    },
    658: {
      id: 658,
      placeName: 2284,
    },
    659: {
      id: 659,
      placeName: 2403,
    },
    660: {
      id: 660,
      placeName: 2810,
    },
    661: {
      id: 661,
      placeName: 2665,
    },
    662: {
      id: 662,
      placeName: 2298,
    },
    663: {
      id: 663,
      placeName: 2707,
    },
    664: {
      id: 664,
      placeName: 2404,
    },
    665: {
      id: 665,
      placeName: 2404,
    },
    666: {
      id: 666,
      placeName: 41,
    },
    667: {
      id: 667,
      placeName: 2404,
    },
    668: {
      id: 668,
      placeName: 44,
    },
    669: {
      id: 669,
      placeName: 45,
    },
    670: {
      id: 670,
      placeName: 2406,
    },
    671: {
      id: 671,
      placeName: 2406,
    },
    672: {
      id: 672,
      placeName: 67,
    },
    673: {
      id: 673,
      placeName: 2031,
    },
    674: {
      id: 674,
      placeName: 2295,
    },
    675: {
      id: 675,
      placeName: 33,
    },
    676: {
      id: 676,
      placeName: 2038,
    },
    677: {
      id: 677,
      placeName: 2295,
    },
    678: {
      id: 678,
      placeName: 2406,
    },
    679: {
      id: 679,
      placeName: 2708,
    },
    680: {
      id: 680,
      placeName: 1960,
    },
    681: {
      id: 681,
      placeName: 2805,
    },
    682: {
      id: 682,
      placeName: 2813,
    },
    683: {
      id: 683,
      placeName: 2339,
    },
    684: {
      id: 684,
      placeName: 2408,
    },
    685: {
      id: 685,
      placeName: 2410,
    },
    686: {
      id: 686,
      placeName: 2408,
    },
    687: {
      id: 687,
      placeName: 2408,
    },
    688: {
      id: 688,
      placeName: 2411,
    },
    689: {
      id: 689,
      placeName: 2691,
    },
    690: {
      id: 690,
      placeName: 2284,
    },
    691: {
      id: 691,
      placeName: 2357,
    },
    692: {
      id: 692,
      placeName: 2358,
    },
    693: {
      id: 693,
      placeName: 2359,
    },
    694: {
      id: 694,
      placeName: 2360,
    },
    695: {
      id: 695,
      placeName: 2357,
    },
    696: {
      id: 696,
      placeName: 2358,
    },
    697: {
      id: 697,
      placeName: 2359,
    },
    698: {
      id: 698,
      placeName: 2360,
    },
    699: {
      id: 699,
      placeName: 63,
    },
    700: {
      id: 700,
      placeName: 2300,
    },
    701: {
      id: 701,
      placeName: 496,
    },
    702: {
      id: 702,
      placeName: 2147,
    },
    703: {
      id: 703,
      placeName: 2406,
    },
    704: {
      id: 704,
      placeName: 1301,
    },
    705: {
      id: 705,
      placeName: 41,
    },
    706: {
      id: 706,
      placeName: 41,
    },
    707: {
      id: 707,
      placeName: 1742,
    },
    708: {
      id: 708,
      placeName: 462,
    },
    709: {
      id: 709,
      placeName: 2200,
    },
    710: {
      id: 710,
      placeName: 2404,
    },
    711: {
      id: 711,
      placeName: 2409,
    },
    712: {
      id: 712,
      placeName: 2340,
    },
    713: {
      id: 713,
      placeName: 2411,
    },
    714: {
      id: 714,
      placeName: 2833,
    },
    715: {
      id: 715,
      placeName: 2002,
    },
    716: {
      id: 716,
      placeName: 2407,
    },
    717: {
      id: 717,
      placeName: 358,
    },
    718: {
      id: 718,
      placeName: 2411,
    },
    719: {
      id: 719,
      placeName: 2299,
    },
    720: {
      id: 720,
      placeName: 2299,
    },
    721: {
      id: 721,
      placeName: 128,
    },
    722: {
      id: 722,
      placeName: 125,
    },
    723: {
      id: 723,
      placeName: 2411,
    },
    724: {
      id: 724,
      placeName: 2284,
    },
    725: {
      id: 725,
      placeName: 2340,
    },
    726: {
      id: 726,
      placeName: 2409,
    },
    727: {
      id: 727,
      placeName: 2709,
    },
    728: {
      id: 728,
      placeName: 153,
    },
    729: {
      id: 729,
      placeName: 2099,
    },
    730: {
      id: 730,
      placeName: 2715,
    },
    731: {
      id: 731,
      placeName: 2367,
    },
    732: {
      id: 732,
      placeName: 2414,
    },
    733: {
      id: 733,
      placeName: 1759,
    },
    734: {
      id: 734,
      placeName: 2372,
    },
    735: {
      id: 735,
      placeName: 2370,
    },
    736: {
      id: 736,
      placeName: 2371,
    },
    737: {
      id: 737,
      placeName: 2294,
    },
    738: {
      id: 738,
      placeName: 2296,
    },
    739: {
      id: 739,
      placeName: 2813,
    },
    740: {
      id: 740,
      placeName: 2709,
    },
    741: {
      id: 741,
      placeName: 112,
    },
    742: {
      id: 742,
      placeName: 2762,
    },
    743: {
      id: 743,
      placeName: 2148,
    },
    744: {
      id: 744,
      placeName: 2847,
    },
    745: {
      id: 745,
      placeName: 2383,
    },
    746: {
      id: 746,
      placeName: 2354,
    },
    747: {
      id: 747,
      placeName: 0,
    },
    748: {
      id: 748,
      placeName: 2717,
    },
    749: {
      id: 749,
      placeName: 2718,
    },
    750: {
      id: 750,
      placeName: 2719,
    },
    751: {
      id: 751,
      placeName: 2720,
    },
    752: {
      id: 752,
      placeName: 2717,
    },
    753: {
      id: 753,
      placeName: 2718,
    },
    754: {
      id: 754,
      placeName: 2719,
    },
    755: {
      id: 755,
      placeName: 2720,
    },
    756: {
      id: 756,
      placeName: 2284,
    },
    757: {
      id: 757,
      placeName: 2409,
    },
    758: {
      id: 758,
      placeName: 2354,
    },
    759: {
      id: 759,
      placeName: 2813,
    },
    760: {
      id: 760,
      placeName: 2406,
    },
    761: {
      id: 761,
      placeName: 2448,
    },
    762: {
      id: 762,
      placeName: 2448,
    },
    763: {
      id: 763,
      placeName: 2462,
    },
    764: {
      id: 764,
      placeName: 2392,
    },
    765: {
      id: 765,
      placeName: 2383,
    },
    766: {
      id: 766,
      placeName: 2383,
    },
    767: {
      id: 767,
      placeName: 2383,
    },
    768: {
      id: 768,
      placeName: 2801,
    },
    769: {
      id: 769,
      placeName: 2851,
    },
    770: {
      id: 770,
      placeName: 2775,
    },
    771: {
      id: 771,
      placeName: 2775,
    },
    772: {
      id: 772,
      placeName: 2775,
    },
    773: {
      id: 773,
      placeName: 2775,
    },
    774: {
      id: 774,
      placeName: 2775,
    },
    775: {
      id: 775,
      placeName: 2775,
    },
    776: {
      id: 776,
      placeName: 2483,
    },
    777: {
      id: 777,
      placeName: 2449,
    },
    778: {
      id: 778,
      placeName: 2450,
    },
    779: {
      id: 779,
      placeName: 2450,
    },
    780: {
      id: 780,
      placeName: 2775,
    },
    781: {
      id: 781,
      placeName: 2391,
    },
    782: {
      id: 782,
      placeName: 2775,
    },
    783: {
      id: 783,
      placeName: 2775,
    },
    784: {
      id: 784,
      placeName: 2775,
    },
    785: {
      id: 785,
      placeName: 2775,
    },
    786: {
      id: 786,
      placeName: 2799,
    },
    787: {
      id: 787,
      placeName: 2451,
    },
    788: {
      id: 788,
      placeName: 2034,
    },
    789: {
      id: 789,
      placeName: 2851,
    },
    790: {
      id: 790,
      placeName: 40,
    },
    791: {
      id: 791,
      placeName: 2589,
    },
    792: {
      id: 792,
      placeName: 2498,
    },
    793: {
      id: 793,
      placeName: 2586,
    },
    794: {
      id: 794,
      placeName: 2485,
    },
    795: {
      id: 795,
      placeName: 2530,
    },
    796: {
      id: 796,
      placeName: 2548,
    },
    797: {
      id: 797,
      placeName: 2411,
    },
    798: {
      id: 798,
      placeName: 2725,
    },
    799: {
      id: 799,
      placeName: 2736,
    },
    800: {
      id: 800,
      placeName: 2727,
    },
    801: {
      id: 801,
      placeName: 2737,
    },
    802: {
      id: 802,
      placeName: 2725,
    },
    803: {
      id: 803,
      placeName: 2736,
    },
    804: {
      id: 804,
      placeName: 2727,
    },
    805: {
      id: 805,
      placeName: 2737,
    },
    806: {
      id: 806,
      placeName: 2499,
    },
    807: {
      id: 807,
      placeName: 2284,
    },
    808: {
      id: 808,
      placeName: 2284,
    },
    809: {
      id: 809,
      placeName: 1834,
    },
    810: {
      id: 810,
      placeName: 2496,
    },
    811: {
      id: 811,
      placeName: 2496,
    },
    812: {
      id: 812,
      placeName: 2284,
    },
    813: {
      id: 813,
      placeName: 2953,
    },
    814: {
      id: 814,
      placeName: 2954,
    },
    815: {
      id: 815,
      placeName: 2955,
    },
    816: {
      id: 816,
      placeName: 2956,
    },
    817: {
      id: 817,
      placeName: 2957,
    },
    818: {
      id: 818,
      placeName: 2958,
    },
    819: {
      id: 819,
      placeName: 2951,
    },
    820: {
      id: 820,
      placeName: 2952,
    },
    821: {
      id: 821,
      placeName: 2979,
    },
    822: {
      id: 822,
      placeName: 2997,
    },
    823: {
      id: 823,
      placeName: 3018,
    },
    824: {
      id: 824,
      placeName: 2510,
    },
    825: {
      id: 825,
      placeName: 2510,
    },
    826: {
      id: 826,
      placeName: 2864,
    },
    827: {
      id: 827,
      placeName: 2545,
    },
    828: {
      id: 828,
      placeName: 2370,
    },
    829: {
      id: 829,
      placeName: 2862,
    },
    830: {
      id: 830,
      placeName: 2586,
    },
    831: {
      id: 831,
      placeName: 2549,
    },
    832: {
      id: 832,
      placeName: 1484,
    },
    833: {
      id: 833,
      placeName: 459,
    },
    834: {
      id: 834,
      placeName: 361,
    },
    835: {
      id: 835,
      placeName: 0,
    },
    836: {
      id: 836,
      placeName: 3139,
    },
    837: {
      id: 837,
      placeName: 3050,
    },
    838: {
      id: 838,
      placeName: 2985,
    },
    839: {
      id: 839,
      placeName: 55,
    },
    840: {
      id: 840,
      placeName: 2982,
    },
    841: {
      id: 841,
      placeName: 3007,
    },
    842: {
      id: 842,
      placeName: 3221,
    },
    843: {
      id: 843,
      placeName: 3222,
    },
    844: {
      id: 844,
      placeName: 3223,
    },
    845: {
      id: 845,
      placeName: 3218,
    },
    846: {
      id: 846,
      placeName: 3219,
    },
    847: {
      id: 847,
      placeName: 3220,
    },
    848: {
      id: 848,
      placeName: 3219,
    },
    849: {
      id: 849,
      placeName: 3214,
    },
    850: {
      id: 850,
      placeName: 3215,
    },
    851: {
      id: 851,
      placeName: 3216,
    },
    852: {
      id: 852,
      placeName: 3217,
    },
    853: {
      id: 853,
      placeName: 3214,
    },
    854: {
      id: 854,
      placeName: 3215,
    },
    855: {
      id: 855,
      placeName: 3216,
    },
    856: {
      id: 856,
      placeName: 3217,
    },
    857: {
      id: 857,
      placeName: 3214,
    },
    858: {
      id: 858,
      placeName: 3218,
    },
    859: {
      id: 859,
      placeName: 3228,
    },
    860: {
      id: 860,
      placeName: 2955,
    },
    861: {
      id: 861,
      placeName: 2953,
    },
    862: {
      id: 862,
      placeName: 2953,
    },
    863: {
      id: 863,
      placeName: 2952,
    },
    864: {
      id: 864,
      placeName: 2954,
    },
    865: {
      id: 865,
      placeName: 53,
    },
    866: {
      id: 866,
      placeName: 2200,
    },
    867: {
      id: 867,
      placeName: 32,
    },
    868: {
      id: 868,
      placeName: 2407,
    },
    869: {
      id: 869,
      placeName: 2956,
    },
    870: {
      id: 870,
      placeName: 2954,
    },
    871: {
      id: 871,
      placeName: 2957,
    },
    872: {
      id: 872,
      placeName: 2955,
    },
    873: {
      id: 873,
      placeName: 3218,
    },
    874: {
      id: 874,
      placeName: 2957,
    },
    875: {
      id: 875,
      placeName: 2957,
    },
    876: {
      id: 876,
      placeName: 3226,
    },
    877: {
      id: 877,
      placeName: 2953,
    },
    878: {
      id: 878,
      placeName: 3225,
    },
    879: {
      id: 879,
      placeName: 3229,
    },
    880: {
      id: 880,
      placeName: 3219,
    },
    881: {
      id: 881,
      placeName: 3220,
    },
    882: {
      id: 882,
      placeName: 3425,
    },
    883: {
      id: 883,
      placeName: 0,
    },
    884: {
      id: 884,
      placeName: 3385,
    },
    885: {
      id: 885,
      placeName: 3220,
    },
    886: {
      id: 886,
      placeName: 3435,
    },
    887: {
      id: 887,
      placeName: 3017,
    },
    888: {
      id: 888,
      placeName: 3378,
    },
    889: {
      id: 889,
      placeName: 3428,
    },
    890: {
      id: 890,
      placeName: 3428,
    },
    891: {
      id: 891,
      placeName: 3428,
    },
    892: {
      id: 892,
      placeName: 3428,
    },
    893: {
      id: 893,
      placeName: 3429,
    },
    894: {
      id: 894,
      placeName: 3428,
    },
    895: {
      id: 895,
      placeName: 3427,
    },
    896: {
      id: 896,
      placeName: 3425,
    },
    897: {
      id: 897,
      placeName: 3442,
    },
    898: {
      id: 898,
      placeName: 3467,
    },
    899: {
      id: 899,
      placeName: 2527,
    },
    900: {
      id: 900,
      placeName: 3477,
    },
    901: {
      id: 901,
      placeName: 1647,
    },
    902: {
      id: 902,
      placeName: 3468,
    },
    903: {
      id: 903,
      placeName: 3469,
    },
    904: {
      id: 904,
      placeName: 3470,
    },
    905: {
      id: 905,
      placeName: 3487,
    },
    906: {
      id: 906,
      placeName: 3468,
    },
    907: {
      id: 907,
      placeName: 3469,
    },
    908: {
      id: 908,
      placeName: 3470,
    },
    909: {
      id: 909,
      placeName: 3487,
    },
    910: {
      id: 910,
      placeName: 0,
    },
    911: {
      id: 911,
      placeName: 3479,
    },
    912: {
      id: 912,
      placeName: 3442,
    },
    913: {
      id: 913,
      placeName: 3486,
    },
    914: {
      id: 914,
      placeName: 3471,
    },
    915: {
      id: 915,
      placeName: 3478,
    },
    916: {
      id: 916,
      placeName: 3511,
    },
    917: {
      id: 917,
      placeName: 3492,
    },
    918: {
      id: 918,
      placeName: 3467,
    },
    919: {
      id: 919,
      placeName: 3571,
    },
    920: {
      id: 920,
      placeName: 3534,
    },
    921: {
      id: 921,
      placeName: 695,
    },
    922: {
      id: 922,
      placeName: 3568,
    },
    923: {
      id: 923,
      placeName: 3568,
    },
    924: {
      id: 924,
      placeName: 3644,
    },
    925: {
      id: 925,
      placeName: 3570,
    },
    926: {
      id: 926,
      placeName: 3570,
    },
    927: {
      id: 927,
      placeName: 0,
    },
    928: {
      id: 928,
      placeName: 3576,
    },
    929: {
      id: 929,
      placeName: 1647,
    },
    930: {
      id: 930,
      placeName: 1399,
    },
    931: {
      id: 931,
      placeName: 3568,
    },
    932: {
      id: 932,
      placeName: 2958,
    },
    933: {
      id: 933,
      placeName: 3590,
    },
    934: {
      id: 934,
      placeName: 3581,
    },
    935: {
      id: 935,
      placeName: 3581,
    },
    936: {
      id: 936,
      placeName: 0,
    },
    937: {
      id: 937,
      placeName: 0,
    },
    938: {
      id: 938,
      placeName: 0,
    },
    939: {
      id: 939,
      placeName: 0,
    },
    940: {
      id: 940,
      placeName: 1665,
    },
    941: {
      id: 941,
      placeName: 1665,
    },
    942: {
      id: 942,
      placeName: 3596,
    },
    943: {
      id: 943,
      placeName: 3620,
    },
    944: {
      id: 944,
      placeName: 3595,
    },
    945: {
      id: 945,
      placeName: 3635,
    },
    946: {
      id: 946,
      placeName: 3596,
    },
    947: {
      id: 947,
      placeName: 3620,
    },
    948: {
      id: 948,
      placeName: 3595,
    },
    949: {
      id: 949,
      placeName: 3635,
    },
    950: {
      id: 950,
      placeName: 0,
    },
    951: {
      id: 951,
      placeName: 0,
    },
    952: {
      id: 952,
      placeName: 0,
    },
    953: {
      id: 953,
      placeName: 359,
    },
    954: {
      id: 954,
      placeName: 359,
    },
    955: {
      id: 955,
      placeName: 0,
    },
    956: {
      id: 956,
      placeName: 0,
    },
    957: {
      id: 957,
      placeName: 0,
    },
    958: {
      id: 958,
      placeName: 0,
    },
    959: {
      id: 959,
      placeName: 0,
    },
    960: {
      id: 960,
      placeName: 0,
    },
    961: {
      id: 961,
      placeName: 0,
    },
    962: {
      id: 962,
      placeName: 0,
    },
    963: {
      id: 963,
      placeName: 0,
    },
    964: {
      id: 964,
      placeName: 0,
    },
    965: {
      id: 965,
      placeName: 3225,
    },
    966: {
      id: 966,
      placeName: 0,
    },
    967: {
      id: 967,
      placeName: 3581,
    },
    968: {
      id: 968,
      placeName: 0,
    },
    969: {
      id: 969,
      placeName: 0,
    },
    970: {
      id: 970,
      placeName: 0,
    },
    971: {
      id: 971,
      placeName: 0,
    },
    972: {
      id: 972,
      placeName: 0,
    },
    973: {
      id: 973,
      placeName: 0,
    },
    974: {
      id: 974,
      placeName: 0,
    },
    975: {
      id: 975,
      placeName: 0,
    },
    976: {
      id: 976,
      placeName: 0,
    },
    977: {
      id: 977,
      placeName: 0,
    },
    978: {
      id: 978,
      placeName: 0,
    },
    979: {
      id: 979,
      placeName: 0,
    },
    980: {
      id: 980,
      placeName: 0,
    },
  },
  AQUARIUM_WATER: {
    1: {
      id: 1,
      name_chs: '淡水',
    },
    2: {
      id: 2,
      name_chs: '海水',
    },
  },
  AQUARIUM_FISH_SIZE: {
    1: {
      id: 1,
      size: 'S',
    },
    2: {
      id: 2,
      size: 'M',
    },
    3: {
      id: 3,
      size: 'L',
    },
    4: {
      id: 4,
      size: 'XL',
    },
  },
  AQUARIUMS: {
    4874: {
      id: 4874,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    4876: {
      id: 4876,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    4879: {
      id: 4879,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    4883: {
      id: 4883,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    4886: {
      id: 4886,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    4893: {
      id: 4893,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    4895: {
      id: 4895,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    4898: {
      id: 4898,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    4903: {
      id: 4903,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    4905: {
      id: 4905,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4908: {
      id: 4908,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    4917: {
      id: 4917,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    4918: {
      id: 4918,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    4922: {
      id: 4922,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    4923: {
      id: 4923,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    4924: {
      id: 4924,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    4926: {
      id: 4926,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    4948: {
      id: 4948,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    4951: {
      id: 4951,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4959: {
      id: 4959,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4970: {
      id: 4970,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    4973: {
      id: 4973,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4979: {
      id: 4979,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4986: {
      id: 4986,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4987: {
      id: 4987,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    4999: {
      id: 4999,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    5000: {
      id: 5000,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    5007: {
      id: 5007,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    5011: {
      id: 5011,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    5013: {
      id: 5013,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    5023: {
      id: 5023,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    5025: {
      id: 5025,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    7685: {
      id: 7685,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    7690: {
      id: 7690,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    7693: {
      id: 7693,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    7699: {
      id: 7699,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    7709: {
      id: 7709,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    7902: {
      id: 7902,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    7924: {
      id: 7924,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    7940: {
      id: 7940,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    7941: {
      id: 7941,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    7943: {
      id: 7943,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    8762: {
      id: 8762,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    8774: {
      id: 8774,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    8776: {
      id: 8776,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    12721: {
      id: 12721,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    12739: {
      id: 12739,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    12741: {
      id: 12741,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12742: {
      id: 12742,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    12743: {
      id: 12743,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12749: {
      id: 12749,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    12750: {
      id: 12750,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12752: {
      id: 12752,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    12764: {
      id: 12764,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12768: {
      id: 12768,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12781: {
      id: 12781,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    12803: {
      id: 12803,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    12830: {
      id: 12830,
      aquariumWater: 1,
      size: 4,
      patch: 5.4,
    },
    17579: {
      id: 17579,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    17590: {
      id: 17590,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    20018: {
      id: 20018,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    20020: {
      id: 20020,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20021: {
      id: 20021,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    20038: {
      id: 20038,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20048: {
      id: 20048,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    20051: {
      id: 20051,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20052: {
      id: 20052,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20053: {
      id: 20053,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20054: {
      id: 20054,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    20090: {
      id: 20090,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20100: {
      id: 20100,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20110: {
      id: 20110,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20145: {
      id: 20145,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20157: {
      id: 20157,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    20162: {
      id: 20162,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    20171: {
      id: 20171,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    20180: {
      id: 20180,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20183: {
      id: 20183,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20184: {
      id: 20184,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    20185: {
      id: 20185,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20186: {
      id: 20186,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    20192: {
      id: 20192,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    20193: {
      id: 20193,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20204: {
      id: 20204,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    20211: {
      id: 20211,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    20218: {
      id: 20218,
      aquariumWater: 1,
      size: 2,
      patch: 5.4,
    },
    20220: {
      id: 20220,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20221: {
      id: 20221,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20226: {
      id: 20226,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    20228: {
      id: 20228,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    20229: {
      id: 20229,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    20233: {
      id: 20233,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    21175: {
      id: 21175,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    21176: {
      id: 21176,
      aquariumWater: 2,
      size: 3,
      patch: 5.2,
    },
    21177: {
      id: 21177,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    21178: {
      id: 21178,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    22389: {
      id: 22389,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    22397: {
      id: 22397,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    22398: {
      id: 22398,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    23054: {
      id: 23054,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    23059: {
      id: 23059,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    24203: {
      id: 24203,
      aquariumWater: 1,
      size: 1,
      patch: 5.4,
    },
    24204: {
      id: 24204,
      aquariumWater: 1,
      size: 1,
      patch: 5.4,
    },
    24990: {
      id: 24990,
      aquariumWater: 1,
      size: 4,
      patch: 5.3,
    },
    24994: {
      id: 24994,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    24995: {
      id: 24995,
      aquariumWater: 1,
      size: 1,
      patch: 5.3,
    },
    27415: {
      id: 27415,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    27423: {
      id: 27423,
      aquariumWater: 2,
      size: 1,
      patch: 5.2,
    },
    27424: {
      id: 27424,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    27430: {
      id: 27430,
      aquariumWater: 1,
      size: 2,
      patch: 5.3,
    },
    27436: {
      id: 27436,
      aquariumWater: 1,
      size: 3,
      patch: 5.4,
    },
    27437: {
      id: 27437,
      aquariumWater: 1,
      size: 4,
      patch: 5.2,
    },
    27454: {
      id: 27454,
      aquariumWater: 2,
      size: 2,
      patch: 5.2,
    },
    27474: {
      id: 27474,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    27481: {
      id: 27481,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    27490: {
      id: 27490,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    27494: {
      id: 27494,
      aquariumWater: 1,
      size: 1,
      patch: 5.3,
    },
    27503: {
      id: 27503,
      aquariumWater: 2,
      size: 4,
      patch: 5.2,
    },
    27530: {
      id: 27530,
      aquariumWater: 1,
      size: 2,
      patch: 5.3,
    },
    27531: {
      id: 27531,
      aquariumWater: 1,
      size: 2,
      patch: 5.2,
    },
    27534: {
      id: 27534,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    27536: {
      id: 27536,
      aquariumWater: 1,
      size: 2,
      patch: 5.3,
    },
    27543: {
      id: 27543,
      aquariumWater: 1,
      size: 1,
      patch: 5.2,
    },
    27546: {
      id: 27546,
      aquariumWater: 1,
      size: 2,
      patch: 5.4,
    },
    27557: {
      id: 27557,
      aquariumWater: 1,
      size: 2,
      patch: 5.4,
    },
    27559: {
      id: 27559,
      aquariumWater: 1,
      size: 3,
      patch: 5.2,
    },
    27560: {
      id: 27560,
      aquariumWater: 1,
      size: 3,
      patch: 5.3,
    },
    27579: {
      id: 27579,
      aquariumWater: 1,
      size: 3,
      patch: 5.4,
    },
    29719: {
      id: 29719,
      aquariumWater: 2,
      size: 2,
      patch: 5.4,
    },
    29784: {
      id: 29784,
      aquariumWater: 2,
      size: 4,
      patch: 5.4,
    },
    29785: {
      id: 29785,
      aquariumWater: 2,
      size: 2,
      patch: 5.3,
    },
    29787: {
      id: 29787,
      aquariumWater: 2,
      size: 3,
      patch: 5.3,
    },
    29790: {
      id: 29790,
      aquariumWater: 2,
      size: 4,
      patch: 5.3,
    },
  },
}
